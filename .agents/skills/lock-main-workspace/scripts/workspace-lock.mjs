...
function parseDuration(value, option) {
  ...
  if (!match)
    fail(`${option} must be a duration such as 500ms, 30s, 20m, or 2h`);
  const multipliers = { ms: 1, s: 1_000, m: 60_000, h: 3_600_000 };
  const result =
    Number(match[1]) * multipliers[(match[2] || "ms").toLowerCase()];
...
function resolveLockDirectory(override, cwd = process.cwd()) {
  ...
  if (process.env.MAIN_WORKSPACE_LOCK_DIR)
    return path.resolve(process.env.MAIN_WORKSPACE_LOCK_DIR);
  ...
  if (!commonDirectory)
    fail("cannot find a Git common directory; use --lock-dir <path>");
...
function resolveMainWorkspace(cwd = process.cwd()) {
  ...
  return firstLine.startsWith("worktree ")
...
async function loadTickets(paths) {
  ...
  for (const name of names.filter((entry) => entry.endsWith(".json"))) {
    ...
    if (ticket?.version === VERSION && ticket.id)
      tickets.push({ ...ticket, file });
  }
  tickets.sort(
    (left, right) =>
...
async function acquire(options) {
  ...
  if (existingHolder === null)
    fail(`holder file is invalid: ${paths.holderFile}`, 1);
  if (existingHolder?.agent === options.agent) {
    process.stdout.write(
      `ALREADY ACQUIRED ${options.agent}\nMain workspace: ${existingHolder.mainWorkspace}\nLease: ${existingHolder.leaseId}\n`,
    );
    return 0;
  }

  await pruneDeadWaiters(paths);
  const existingTicket = (await loadTickets(paths)).find(
    (ticket) => ticket.agent === options.agent,
  );
  if (existingTicket)
    fail(
      `agent ${options.agent} is already waiting in another acquire process`,
    );
  ...
  ticket.file = path.join(
    paths.ticketsDirectory,
    `${String(queuedAtMs).padStart(16, "0")}-${ticket.id}.json`,
  );
  const temporaryTicket = `${ticket.file}.${process.pid}.tmp`;
  await writeFile(
    temporaryTicket,
    `${JSON.stringify(ticket, (key, value) => (key === "file" ? undefined : value), 2)}\n`,
    {
      encoding: "utf8",
      flag: "wx",
    },
  ...
  const log = (message) => {
    if (!options.quiet)
      process.stderr.write(`[workspace-lock] ${options.agent}: ${message}\n`);
  };
  ...
  try {
    ...
    while (true) {
      ...
      if (
        ...
        Date.now() - startedAt >= options.waitTimeoutMs
      ) {
      ...
      if (position === 1 && holder === undefined) {
        const acquiredHolder = await createHolder(
          paths,
          ticket,
      ...
      if (position !== lastPosition) {
        log(
          `waiting at position ${position}${holder ? `; held by ${holder.agent}` : ""}`,
        );
...
async function release(options) {
  ...
  if (holder.agent !== options.agent && !options.force) {
    fail(
      `main workspace is held by ${holder.agent}; ${options.agent} cannot release it`,
    );
  }
...
async function snapshot(options) {
  const paths = lockPaths(resolveLockDirectory(options.lockDir));
  const [holder, tickets] = await Promise.all([
    readJson(paths.holderFile),
    loadTickets(paths),
  ]);
  if (holder === null) fail(`holder file is invalid: ${paths.holderFile}`, 1);
  return {
    ...
    waiters: tickets.map((ticket, index) => ({
      ...
      waiterAlive:
        ticket.hostname !== localHostname || isProcessAlive(ticket.waiterPid),
    })),
  };
}
...
async function showStatus(options) {
  ...
  if (state.holder) {
    process.stdout.write(
      `Holder: ${state.holder.agent} (${formatAge(state.holder.acquiredAtMs)})${state.holder.label ? ` - ${state.holder.label}` : ""}\n`,
    );
  } else {
  ...
  for (const waiter of state.waiters) {
    ...
    process.stdout.write(
      `#${waiter.position} ${waiter.agent} (${formatAge(waiter.queuedAtMs)})${waiter.label ? ` - ${waiter.label}` : ""}${stale}\n`,
    );
  }
  return 0;
}
