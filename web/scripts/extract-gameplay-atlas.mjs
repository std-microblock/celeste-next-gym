...
const listPattern = process.argv.find((argument) => argument.startsWith('--list='))?.slice(7)

if (!source) {
  console.error('usage: node scripts/extract-gameplay-atlas.mjs <Celeste Content/Graphics/Atlases> [output] [--list=regex]')
  process.exit(2)
}
...
class Reader {
  ...
  i16() { const value = this.buffer.readInt16LE(this.offset); this.offset += 2; return value }
  i32() { const value = this.buffer.readInt32LE(this.offset); this.offset += 4; return value }
  string() {
    let length = 0
    let shift = 0
    let byte
  ...
  for (let pageIndex = 0; pageIndex < pageCount; pageIndex += 1) {
    ...
    for (let entryIndex = 0; entryIndex < entryCount; entryIndex += 1) {
      page.entries.push({
        ...
        x: reader.i16(), y: reader.i16(), width: reader.i16(), height: reader.i16(),
        trimX: reader.i16(), trimY: reader.i16(), frameWidth: reader.i16(), frameHeight: reader.i16(),
      })
    }
    pages.push(page)
  }
...
function decodeTexture(buffer) {
  ...
  while (pixel < width * height) {
    ...
    if (run === 0) throw new Error(`invalid zero-length run at ${sourceOffset}`)
    let red = 0; let green = 0; let blue = 0; let opacity = 255
    if (alpha) {
...
const PLAYER_PATHS = [
  'characters/player/idle', 'characters/player/runSlow', 'characters/player/runFast',
  'characters/player/jumpSlow', 'characters/player/jumpFast', 'characters/player/dash',
  'characters/player/climb', 'characters/player/duck', 'characters/player/death_h',
  'characters/player/dreamDash', 'characters/player/swim', 'characters/player/startStarFly',
  'characters/player/startStarFlyWhite', 'characters/player/starFly', 'characters/player/launch',
  'characters/player/hair', 'characters/player/bangs',
]
const GAMEPLAY_PATHS = [
  ...
  'collectables/heartGem/', 'objects/cassetteblock/', 'danger/crystal/',
...
if (listPattern) {
  ...
  for (const page of meta.pages) {
    for (const entry of page.entries) if (matcher.test(entry.name)) console.log(entry.name)
  }
  process.exit(0)
}
if (!destination) throw new Error('output directory is required unless --list is used')

const selected = []
for (const page of meta.pages) {
  for (const entry of page.entries) {
    if (GAMEPLAY_PATHS.some((prefix) => entry.name === prefix || entry.name.startsWith(prefix))) selected.push({ ...entry, page: page.name })
  }
}

const decodedPages = new Map()
for (const pageName of new Set(selected.map((entry) => entry.page))) {
  decodedPages.set(pageName, decodeTexture(await readFile(path.join(source, `${pageName}.data`))))
}
...
for (const entry of placements) {
  ...
  const crop = await sharp(page.pixels, { raw: { width: page.width, height: page.height, channels: 4 } })
...
const manifest = Object.fromEntries(placements.map((entry) => [entry.name, {
  x: entry.sheetX,
  y: entry.sheetY,
  width: entry.width,
  height: entry.height,
...
await writeFile(path.join(destination, 'gameplay-selected.json'), `${JSON.stringify({ sourceHash: meta.sourceHash, entries: manifest }, null, 2)}\n`, 'utf8')
console.log(`Extracted ${placements.length} original Gameplay textures to ${destination}`)
