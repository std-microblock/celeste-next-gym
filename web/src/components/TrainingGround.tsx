...
export function trainingInputLocked(outcome: OutcomeAnimation | null, settlement = false): boolean {
...
export function TrainingGround({ techniqueId, variantId, bindings, theme, onSelectTraining }: { techniqueId: string; variantId: string; bindings: KeyBindings; theme: VisualTheme; onSelectTraining(techniqueId: string, variantId: string): void }) {
  ...
  const predictionRef = useRef<PredictionPreview>({ windows: [], objectives: [] })
  ...
  const applyPrediction = (next: PredictionPreview) => { predictionRef.current = next; setPrediction(next) }
  const clearOutcome = () => {
  ...
  const beginFailure = (timelineFrame: number, candidate?: TrainingCandidate) => {
    const next: OutcomeAnimation = {
      phase: 'failed', startedAt: performance.now(), durationMs: FAILURE_SLOWDOWN_MS,
      objectiveValues: candidate?.objective_values ?? [], timelineFrame,
    }
  ...
  const setActiveModule = (module: TrainingModule | null, atFrame: number | null) => {
  ...
  const activateModule = (module: TrainingModule, atFrame: number) => {
    if (activeModuleRef.current || completionsRef.current.some((completion) => completion.moduleId === module.id)) return
    setActiveModule(module, atFrame)
  }

  const idsInside = (state: SimState, variant: TrainingVariant) => new Set([
    ...variant.training.modules.filter((module) => triggerContainsPlayer(module.trigger, state)).map((module) => module.trigger.id),
    ...(triggerContainsPlayer(variant.training.finish.trigger, state) ? [variant.training.finish.trigger.id] : []),
  ])

  const scanTriggers = (state: SimState, atFrame: number, completed = completionsRef.current) => {
    const inside = idsInside(state, selectedVariant)
    const entered = [...inside].filter((id) => !occupiedTriggers.current.has(id))
    occupiedTriggers.current = inside
    const completedIds = new Set(completed.map((completion) => completion.moduleId))
    ...
    // frame or two later.
    const module = activeModuleRef.current ? undefined : selectedVariant.training.modules.find((candidate) => inside.has(candidate.trigger.id) && !completedIds.has(candidate.id))
    if (module) activateModule(module, atFrame)
    if (entered.includes(selectedVariant.training.finish.trigger.id)) {
      const ready = !selectedVariant.training.finish.require_all_modules || allModulesCompleted(selectedVariant.training, completedIds)
      if (ready) {
        ...
        const remaining = selectedVariant.training.modules.length - completedIds.size
  ...
  const installVariant = async (variant: TrainingVariant) => {
    ...
    for (const module of variant.training.modules) {
      ...
      if (!entry) throw new Error(`训练模块 ${module.id} 的 entry.input_id 未指向可验证输入`)
      if (entry.at !== 0) throw new Error(`训练模块 ${module.id} 的入口输入必须位于本地 F0`)
    }
    setMap(variant.map)
    ...
    const initialModule = moduleAtPlayer(variant.training, variant.initial, new Set())
    if (initialModule) activateModule(initialModule, 0)
    setFollowReference(true)
    setNotice(initialModule ? `${initialModule.tutorial.title} 已触发；下一个动作将作为 F0。` : `地图已加载；前往不可见训练触发区。`)
  }

  const capturePrediction = (source: TrainingSession, start: number | null, tutorial: TrainingDocument, sourceEvaluations = evaluationsRef.current) => {
    ...
    applyPrediction({
      ...
      windows: candidateWindow(source.candidates, inputIndex).map((window) => ({ from: start + window.from, to: start + window.to })),
      objectives: tutorial.fuzz.objectives.map((objective, objectiveIndex) => ({
        expression: objective.expression,
        points: candidateObjectivePoints(sourceEvaluations, inputIndex).map((point) => ({
          frame: start + point.frame, value: point.values[objectiveIndex], successful: point.successful,
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (active) setNotice(error instanceof Error ? error.message : '训练地图加载失败')
      }
    })()
    return () => { active = false }
  ...
  const seek = (requested: number) => {
    ...
    if (!tutorial || start === null || next < start || candidatesRef.current.length === 0) return
    const replay = attempts.current.filter((attempt) => attempt.frame >= start && attempt.frame <= next).map((attempt) => ({ ...attempt, frame: attempt.frame - start }))
    applySession(rebuildTrainingSession(tutorial, candidatesRef.current, replay))
  }
  ...
  const resetTo = (target = resetFrame) => {
    ...
    setNotice(module ? `已回到 ${module.tutorial.title} 的 R 点 F${target}` : `已回到 R 点 F${target}`)
  }
  ...
  useEffect(() => {
    const down = (event: KeyboardEvent) => {
      ...
      if (resetInput && !event.repeat) { resetTo(); return }
      if (trainingInputLocked(outcomeRef.current, settlementRef.current)) return
      keys.current.add(event.code)
      if (gameInput) {
        setFollowReference(true)
        if (predictionDirty.current) { predictionDirty.current = false; applyPrediction({ windows: [], objectives: [] }) }
        setPlaying(true)
      }
    }
    const up = (event: KeyboardEvent) => {
      if (!trainingInputLocked(outcomeRef.current, settlementRef.current)) keys.current.delete(event.code)
    }
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      const fuzzInputIndex = tutorial ? currentTrainingInput(sourceSession, tutorial)?.fuzzInputIndex : undefined
      const target = fuzzInputIndex === undefined ? undefined : nextTargetFrame(sourceSession.candidates, fuzzInputIndex)
      const fuzzFrame = fuzzStartRef.current === null ? 0 : Math.max(0, frameRef.current - fuzzStartRef.current)
      const assisted = tutorial && autoSlowdown
        ? assistedRate(baseRate, fuzzFrame, target, tutorial.assist.auto_slowdown.radius_frames, Math.max(MAX_AUTO_SLOWDOWN_REDUCTION, tutorial.assist.auto_slowdown.minimum_multiplier))
        : baseRate
      ...
      } else if (tutorial) capturePrediction(sourceSession, fuzzStartRef.current, tutorial)
      carry += Math.min(250, now - last) * 60 / 1000 * rate
      last = now
      if (carry >= 1 && !simulating.current) {
        carry -= 1
        const currentFrame = frameRef.current
        const current = activeOutcome === null ? buttonsFromKeyboard(keys.current, bindings) : outcomeButtons.current
        ...
        void (async () => {
          ...
          const expectedInput = activeTutorial ? currentTrainingInput(beforeSession, activeTutorial) : undefined
          const shouldVerify = activeTutorial ? trainingVerificationTriggered(current, previous, expectedInput) : false
          if (shouldVerify && beforeSession.phase === 'pre_fuzz' && candidatesRef.current.length === 0) {
            const beforeState = snapshotsRef.current[currentFrame]!
            const result = await client.fuzzSearch(beforeState, JSON.stringify(activeTutorial!.fuzz), map)
            if (result.candidates.length === 0) throw new Error(`${activeTutorial!.title} 在当前触发状态下没有成功候选`)
            candidatesRef.current = result.candidates
            evaluationsRef.current = result.evaluations
            setCandidates(result.candidates)
            setEvaluations(result.evaluations)
            beforeSession = createTrainingSession(result.candidates, activeTutorial)
            applySession(beforeSession)
          }
          const trace = await client.simulate(snapshotsRef.current[currentFrame]!, [input], map)
          if (!active) return
          ...
          if (shouldVerify && activeTutorial && activeModule) {
            ...
            const entryPassed = beforeSession.phase === 'pre_fuzz'
              ? trainingEntryContextPassed(current, activeTutorial) && await client.entryCheck(after, activeTutorial.entry.check)
              : true
            const nextSession = verifyTrainingInput(beforeSession, activeTutorial, localFrame, semanticKeys, entryPassed)
            const evaluatedCandidate = matchingTrainingCandidate(evaluationsRef.current, activeTutorial, [
              ...beforeSession.actualInputs, { frame: localFrame, keys: semanticKeys },
            ])
            ...
            if (nextSession.phase !== 'success') capturePrediction(nextSession.phase === 'failed' ? beforeSession : nextSession, fuzzStartRef.current ?? currentFrame, activeTutorial)
            applySession(nextSession)
            if (nextSession.phase === 'failed') {
              setNotice(nextSession.failure?.kind === 'input_order_mismatch' ? activeTutorial.teaching.steps[nextSession.nextVerifiedInput]?.order_error.body ?? '输入顺序不正确。' : nextSession.failure?.kind === 'timing_window_miss' ? activeTutorial.teaching.steps[nextSession.nextVerifiedInput]?.window_error.body ?? '错过输入窗口。' : activeTutorial.entry.failure.body)
              ...
              completionsRef.current = [...completionsRef.current.filter((item) => item.moduleId !== completion.moduleId), completion]
          ...
          if (active) { setPlaying(false); setNotice(error instanceof Error ? error.message : '训练模拟失败') }
        }).finally(() => { simulating.current = false })
      }
  ...
  const activeModule = selectedVariant.training.modules.find((module) => module.id === activeModuleId)
  const tutorial = activeModule?.tutorial ?? null
  const state = snapshots[frame] ?? snapshots.at(-1) ?? initial
  const fuzzInputIndex = tutorial ? currentTrainingInput(session, tutorial)?.fuzzInputIndex : undefined
  const target = fuzzInputIndex === undefined ? undefined : nextTargetFrame(session.candidates, fuzzInputIndex)
  const prompt = tutorial ? session.phase === 'pre_fuzz' ? tutorial.entry.hint : tutorial.teaching.steps[session.nextVerifiedInput]?.prompt ?? '继续保持。' : ''
  const effective = (tutorial && autoSlowdown ? assistedRate(baseRate, fuzzStartFrame === null ? 0 : frame - fuzzStartFrame, target, tutorial.assist.auto_slowdown.radius_frames, Math.max(MAX_AUTO_SLOWDOWN_REDUCTION, tutorial.assist.auto_slowdown.minimum_multiplier)) : baseRate) * (1 - outcomeProgress)
  ...
  return <main className="training-workspace">
    ...
    <section className="training-stage panel-frame">
      ...
      <GameView map={map} state={state} states={snapshots} frame={frame} stale={false} theme={theme}>
        {(viewport) => tutorial && <TrainingPrompt map={map} state={state} viewport={viewport} text={prompt} hidden={outcome !== null || settlement} />}
      </GameView>
      ...
      {outcome && tutorial && <div className="training-failure" style={{ '--outcome-progress': outcomeProgress } as CSSProperties}>
        <div className="training-outcome-layout failed"><div className="training-result-card failed">
          <div className="training-result-heading"><div><small>ATTEMPT RESULT</small><strong>{session.failure?.kind === 'entry_check_failed' ? tutorial.entry.failure.title : session.failure?.kind === 'input_order_mismatch' ? tutorial.teaching.steps[session.nextVerifiedInput]?.order_error.title : tutorial.teaching.steps[session.nextVerifiedInput]?.window_error.title}</strong></div><em>{timing}</em></div>
          ...
          <div className="training-result-stats">
            {tutorial.fuzz.objectives.map((objective, index) => <div key={`${objective.expression}-${index}`}><span>本次 OBJECTIVE · {objective.expression}</span><b>{outcome.objectiveValues[index] === undefined ? '—' : outcome.objectiveValues[index].toFixed(2)}</b></div>)}
            <div><span>输入时机</span><b>{timing}</b></div>
          </div>
          <p>{notice}</p><div className="training-result-actions"><button className="primary" onClick={() => resetTo()}>R 重试</button></div>
        </div></div>
      </div>}
      ...
      {settlement && <div className="training-settlement" role="dialog" aria-modal="true" aria-label="训练地图结算">
        ...
        <section className="training-settlement-card">
          ...
          <div className="training-summary-stats">
            <div><span>平均反应速度</span><b>{(averageReactionFrames * 1000 / 60).toFixed(0)} ms</b></div>
            <div><span>完成模块</span><b>{completions.length}/{selectedVariant.training.modules.length}</b></div>
            <div><span>总用时</span><b>{(frame / 60).toFixed(2)} s</b></div>
            <div><span>最佳精准度</span><b>{Math.max(...completions.map((completion) => completion.accuracy), 0).toFixed(0)}%</b></div>
          </div>
          <div className="training-result-actions"><button onClick={() => void installVariant(selectedVariant)}>重试</button>{nextVariant && <button className="primary" onClick={() => onSelectTraining(technique.id, nextVariant.id)}>下一关</button>}</div>
        </section>
        <aside className="training-recommendations" aria-label="推荐地图">
          <div className="training-recommendations-heading"><small>KEEP CLIMBING</small><h2>推荐地图</h2><p>继续挑战其他训练地图。</p></div>
          <div className="training-recommendation-list">{recommendations.map((variant) => <button type="button" key={variant.id} onClick={() => onSelectTraining(technique.id, variant.id)}><TrainingVariantThumbnail variant={variant} /><span><small>{variant.training.modules.length} 个训练模块</small><strong>{variant.title}</strong><em>{variant.summary}</em><b>开始训练 <i aria-hidden="true">→</i></b></span></button>)}</div>
        </aside>
      </div>}
