...
export function GameplayStrawberry({ scale = 5 }: { scale?: number }) {
  ...
  useEffect(() => {
    ...
    void loadGameplayAtlas().then((loaded) => {
      ...
      const draw = (now: number) => {
        ...
        const entry = atlas.entries[`collectables/strawberry/normal${String(frame).padStart(2, '0')}`]
        ...
        if (entry && context) {
          ...
          context.drawImage(
            ...
            entry.drawOffsetX * scale, entry.drawOffsetY * scale, entry.width * scale, entry.height * scale,
          )
        }
  ...
  return <canvas ref={canvas} className="training-strawberry-sprite" role="img" aria-label="草莓" />
}
