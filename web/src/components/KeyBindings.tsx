  ...
  return <div className="modal-backdrop" onPointerDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="bindings-modal panel-frame" role="dialog" aria-modal="true" aria-labelledby="bindings-title">
      <div className="panel-heading">
        <div><small>CONTROLS</small><h2 id="bindings-title">控制设置</h2></div>
      ...
      <div className="gamepad-settings">
        <div className="gamepad-heading">
          <div><small>GAMEPAD</small><strong>手柄</strong></div>
          <span className={gamepadName ? 'connected' : ''}>{!gamepadSupported ? '浏览器不支持' : gamepadName ? '已连接' : '等待连接'}</span>
        </div>
        <label>
          <span>方向输入</span>
          <select
            ...
            onChange={(event) => onGamepadDirectionSourceChange(event.target.value as GamepadDirectionSource)}
          >
