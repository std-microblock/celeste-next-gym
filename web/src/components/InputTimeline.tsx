...
export const InputTimeline = memo(function InputTimeline({
  ...
  const maxScrollFor = useCallback(
    (width: number, size = cellSize) =>
      Math.max(0, (frameCount + 1) * size - Math.max(1, width - LABEL_WIDTH)),
    [cellSize, frameCount],
  );

  const clampScroll = useCallback(
    (value: number, width: number, size = cellSize) => {
      scrollRef.current = Math.max(
        0,
        Math.min(maxScrollFor(width, size), value),
      );
  ...
  const zoomTo = useCallback(
    (requested: number, anchorCanvasX?: number) => {
      ...
      const anchor = Math.max(
        ...
        Math.min(
          visibleWidth,
          (anchorCanvasX ?? LABEL_WIDTH + visibleWidth / 2) - LABEL_WIDTH,
        ),
      );
      ...
      scrollRef.current = Math.max(
        0,
        Math.min(
          maxScrollFor(canvas.clientWidth, next),
          anchorFrame * next - anchor,
        ),
      );
      setCellSize(next);
      setViewportRevision((revision) => revision + 1);
  ...
  useEffect(() => {
    ...
    const wheel = (event: WheelEvent) => {
      ...
      if (event.ctrlKey)
        zoomTo(
          cellSize - Math.sign(event.deltaY) * 3,
  ...
  useEffect(() => {
    ...
    if (
      playhead < scrollRef.current ||
      playhead + cellSize > scrollRef.current + visibleWidth
    ) {
      clampScroll(playhead - visibleWidth * 0.35, canvas.clientWidth);
    }
  ...
  useEffect(() => {
    ...
    const lastFrame = Math.min(
      frameCount,
      Math.ceil((scroll + visibleWidth) / cellSize) + 1,
    );
    ...
    const viewportWidth = Math.max(
      18,
      Math.min(overviewWidth, (visibleWidth / contentWidth) * overviewWidth),
    );
    const overviewPlayheadX =
      overviewX + (frame / Math.max(1, frameCount)) * overviewWidth;
    ...
    context.rect(
      ...
      0,
      visibleWidth,
      CANVAS_HEIGHT - SCROLLBAR_HEIGHT,
    );
    context.clip();
    ...
    for (const [start, end] of cachedRuns(states, 0, states.length)) {
      ...
      const runWidth = Math.max(
        1,
        ((end - start) / Math.max(1, frameCount)) * overviewWidth,
      );
    ...
    context.strokeRect(
      ...
      overviewTrackHeight - 1,
    );
    context.fillStyle = "#ffffff";
    context.fillRect(
      Math.round(overviewPlayheadX),
      overviewY - 4,
      3,
      overviewTrackHeight + 8,
    );
    context.beginPath();
    ...
    for (let value = firstFrame; value <= lastFrame; value += 1) {
      ...
      if (cellSize >= 10) {
        ...
        context.lineWidth = 1;
        context.beginPath();
        context.moveTo(Math.round(x) + 0.5, OVERVIEW_HEIGHT);
        context.lineTo(Math.round(x) + 0.5, HEADER_HEIGHT + ROWS_HEIGHT);
        context.stroke();
      }
      if (value % (cellSize < 8 ? 50 : cellSize < 16 ? 20 : 10) === 0) {
        ...
        context.fillText(
          String(value),
          x + 3,
          OVERVIEW_HEIGHT + RULER_HEIGHT / 2,
        );
      }
    }
    ...
    for (let lane = 0; lane < LANE_COUNT; lane += 1) {
      ...
      context.beginPath();
      context.moveTo(LABEL_WIDTH, y + ROW_HEIGHT - 0.5);
    ...
    for (let lane = 0; lane < ACTIONS.length; lane += 1) {
      ...
      const y = HEADER_HEIGHT + lane * ROW_HEIGHT;
      const scanFrom =
        firstFrame > 0 && inputs[firstFrame - 1]?.[action]
          ? firstFrame - 1
          : firstFrame;
      for (const [start, end] of heldRuns(
        inputs,
        action,
        scanFrom,
        lastFrame + 1,
    ...
    if (selection) {
      ...
      context.strokeRect(
        Math.round(x) + 2,
        y + 3,
        Math.max(2, Math.round(runWidth) - 4),
        ROW_HEIGHT - 6,
      );
      context.setLineDash([]);
      context.fillStyle = "#ffffff";
    ...
    context.stroke();
    context.setLineDash([]);
    for (const [start, end] of cachedRuns(
      states,
    ...
    while (stateCursor <= lastFrame && stateCursor < states.length) {
      ...
      while (
        stateCursor <= lastFrame &&
        states[stateCursor] &&
        stateColor(states[stateCursor]!) === color
      )
        stateCursor += 1;
      const x = LABEL_WIDTH + start * cellSize - scroll;
      context.fillStyle = color;
    ...
    context.fillRect(
      ...
      OVERVIEW_HEIGHT,
      cellSize,
      RULER_HEIGHT + ROWS_HEIGHT,
    );
    context.fillStyle = "#fff";
    context.fillRect(
      Math.round(playheadX),
      OVERVIEW_HEIGHT,
      2,
      RULER_HEIGHT + ROWS_HEIGHT,
    );
    ...
    context.lineTo(LABEL_WIDTH - 0.5, CANVAS_HEIGHT - SCROLLBAR_HEIGHT);
    context.stroke();
    ...
    const thumbX =
      LABEL_WIDTH + 4 + (maxScroll ? (scroll / maxScroll) * thumbTravel : 0);
  ...
  const pointerFrame = (
    ...
    return Math.max(
      ...
      Math.min(
        frameCount,
        Math.floor(
          (event.clientX - rect.left - LABEL_WIDTH + scrollRef.current) /
            cellSize,
        ),
      ),
    );
  ...
  const overviewFrame = (
    ...
    const ratio = Math.max(
      0,
      Math.min(1, (event.clientX - rect.left - overviewX) / overviewWidth),
    );
  ...
  return (
    <section className="timeline-panel panel-frame">
      <div className="timeline-toolbar">
        <div>
          <small>INPUT EDITOR / CANVAS</small>
          <h2>逐帧时间线</h2>
        </div>
        <div className="timeline-tools">
          <span>
            {states.filter(Boolean).length}/{states.length} STATES
          </span>
          <label>
            容量
            <input
          ...
          <span className="timeline-zoom">
            Ctrl + 滚轮缩放 <output>{cellSize}px</output>
          </span>
        </div>
      </div>
      <canvas
        ...
        onPointerDown={(event) => {
          ...
          if (y < OVERVIEW_HEIGHT) {
            ...
            const pointerX =
              overviewX + (frame / Math.max(1, frameCount)) * trackWidth;
            ...
            if (Math.abs(x - pointerX) <= PLAYHEAD_HIT_RADIUS) {
              ...
              clampScroll(
                clickedFrame * cellSize - visibleWidth / 2,
                rect.width,
              );
              dragRef.current = {
                mode: "overview",
          ...
          if (run) {
            ...
            setSelection(
              paintValue ? { action, start: value, end: value + 1 } : null,
            );
            onSeek(value);
          }
        }}
        onPointerMove={(event) => {
          ...
          if (!drag) {
            if (x < LABEL_WIDTH) event.currentTarget.style.cursor = "default";
              ...
              event.currentTarget.style.cursor =
                Math.abs(x - pointerX) <= PLAYHEAD_HIT_RADIUS
                  ? "ew-resize"
                  : "grab";
            } else if (y < HEADER_HEIGHT) {
              const pointerX =
                LABEL_WIDTH + frame * cellSize - scrollRef.current;
              event.currentTarget.style.cursor =
                Math.abs(x - pointerX) <= PLAYHEAD_HIT_RADIUS
                  ? "ew-resize"
                  : "grab";
            } else if (y >= HEADER_HEIGHT + ROWS_HEIGHT)
              ...
              event.currentTarget.style.cursor =
                ...
                heldRunAt(
                  inputs,
                  action,
                  Math.min(frameCount - 1, pointerFrame(event)),
                )
                  ? "grab"
                  : action
                    ? "crosshair"
          ...
          if (drag.mode === "playhead")
            onSeek(
              drag.source === "overview"
                ? overviewFrame(event)
                : pointerFrame(event),
            );
          else if (drag.mode === "pan")
            clampScroll(drag.startScroll - (x - drag.startX), rect.width);
          else if (drag.mode === "overview")
            clampScroll(
              drag.startScroll +
                ((x - drag.startX) / drag.trackWidth) * drag.contentWidth,
              rect.width,
            );
          else if (drag.mode === "scrollbar")
            clampScroll(
              drag.startScroll +
                ((x - drag.startX) / drag.thumbTravel) * drag.maxScroll,
              rect.width,
            );
            ...
            if (drag.value)
              setSelection({
                ...
                end: Math.max(drag.anchor, current) + 1,
              });
          } else {
            const lane = Math.max(
              0,
              Math.min(
                ACTIONS.length - 1,
                Math.floor((y - HEADER_HEIGHT) / ROW_HEIGHT),
              ),
            );
            ...
            const desired = Math.max(
              ...
              Math.min(
                frameCount - drag.end,
                pointerFrame(event) - drag.anchor,
              ),
            );
