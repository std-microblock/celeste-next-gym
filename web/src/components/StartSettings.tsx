  ...
  return {
    x: Math.round(
      Math.min(
        ...
        Math.max(map.bounds.x, position.x),
      ),
    ),
    y: Math.round(
      Math.min(
        map.bounds.y + map.bounds.height,
        Math.max(map.bounds.y, position.y),
      ),
    ),
  };
}
...
export function StartSettings({
  ...
  const initialRoom =
    rooms.find((candidate) => roomName(candidate) === room) ?? rooms[0];
  const [draftRoom, setDraftRoom] = useState(
    initialRoom ? roomName(initialRoom) : room,
  );
  const [draftPosition, setDraftPosition] = useState<Vec2>({ ...position });
  const selectedMap = useMemo(
    () =>
      rooms.find((candidate) => roomName(candidate) === draftRoom) ??
      initialRoom,
  ...
  const selectPoint = (event: PointerEvent<SVGSVGElement>) => {
    ...
    setDraftPosition(
      clampPosition(selectedMap, {
        x:
          selectedMap.bounds.x +
          ((event.clientX - bounds.left) / bounds.width) *
            selectedMap.bounds.width,
        y:
          selectedMap.bounds.y +
          ((event.clientY - bounds.top) / bounds.height) *
            selectedMap.bounds.height,
      }),
    );
  };
  ...
  return (
    ...
    >
      <section
        className="start-modal panel-frame"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="panel-heading">
          <div>
            <small>SIMULATION ORIGIN</small>
            <h2 id="start-settings-title">选择开始位置</h2>
          </div>
          <button
            ...
            {selectedMap && (
              <div className="start-preview">
                <div className="start-preview-heading">
                  <div>
                    <small>点击地图选择</small>
                    <strong>{roomLabel(roomName(selectedMap))}</strong>
                  </div>
                  <output aria-label="已选坐标">
                ...
                <svg
                  className="start-map"
                  style={{
                    aspectRatio: `${selectedMap.bounds.width} / ${selectedMap.bounds.height}`,
                  }}
                ...
                >
                  <rect
                    ...
                    x={selectedMap.bounds.x}
                    y={selectedMap.bounds.y}
                    width={selectedMap.bounds.width}
                    height={selectedMap.bounds.height}
                  />
                  {selectedMap.solids.map((solid, index) => (
                    <rect
                      className="map-solid"
                      key={`solid-${index}`}
                      {...solid}
                  ...
                  {selectedMap.entities.map((entity, index) => (
                    <rect
                  ...
                  <g
                    className="default-spawn"
                    transform={`translate(${selectedMap.spawn.x} ${selectedMap.spawn.y})`}
                  >
                    <circle r="7" />
                    <path d="M -4 0 H 4 M 0 -4 V 4" />
                  </g>
                  <g
                    className="selected-spawn"
                    transform={`translate(${draftPosition.x} ${draftPosition.y})`}
                  >
                    <circle r="10" />
                ...
                <button
                  className="default-spawn-button"
                  type="button"
                  disabled={busy}
                  onClick={() => setDraftPosition({ ...selectedMap.spawn })}
                >
                  使用房间默认出生点
                </button>
              </div>
