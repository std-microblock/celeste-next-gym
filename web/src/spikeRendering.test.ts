...
describe("spike rendering", () => {
  ...
  it("anchors each sprite to the entity position outside its three-pixel collider", () => {
    ...
    expect(spikePlacement(horizontal, "up", 0, 8, 9)).toEqual({
      x: 14,
      y: 24,
      originX: 4,
      originY: 9,
    });
    expect(spikePlacement(horizontal, "down", 0, 8, 9)).toEqual({
      x: 14,
      y: 19,
      originX: 4,
      originY: 0,
    });
    expect(spikePlacement(vertical, "left", 0, 9, 8)).toEqual({
      x: 14,
      y: 24,
      originX: 9,
      originY: 4,
    });
    expect(spikePlacement(vertical, "right", 0, 9, 8)).toEqual({
      x: 9,
      y: 24,
      originX: 0,
      originY: 4,
