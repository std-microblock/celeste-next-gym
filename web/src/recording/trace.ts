...
export function initialStateFromTrace(
  state: PortableState,
  map: GymMap,
): SimState {
  ...
  return {
    ...
    facing:
      state.facing === true || state.facing === "Right" || state.facing === 1,
    dashes: state.dashes,
...
function validatePortableState(
  value: unknown,
  frame: number,
): asserts value is PortableState {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new Error(`states[${frame}] 不是对象`);
  const state = value as Partial<PortableState>;
  if (!vector(state.pos) || !vector(state.speed))
    throw new Error(`states[${frame}] 的位置或速度无效`);
  if (typeof state.state !== "string" && !Number.isInteger(state.state))
    throw new Error(`states[${frame}].state 无效`);
  if (
