...
const spikeMap = room("Hyper · 越过尖刺", "hyper-spikes", {
  ...
  entities: [
    {
      kind: "spikes",
      bounds: { x: 174, y: 237, width: 80, height: 3 },
      direction: { x: 0, y: -1 },
      name: "spikesUp",
    },
  ],
...
const bubbleInitial = snapshot(bubbleMap.spawn, {
  state: "Boost",
  on_ground: false,
  boost_target: { x: 112, y: 226 },
  last_booster_target: { x: 112, y: 226 },
  booster_reuse_timer: 0.45,
  state_timer: 0.28,
});
...
function variant(
  id: string,
  map: ReturnType<typeof room>,
  training: TrainingMapDocument,
  initial = snapshot(map.spawn),
): TrainingVariant {
  return {
    id,
