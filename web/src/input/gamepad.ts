...
function buttonPressed(gamepad: GamepadSnapshot, index: number): boolean {
  ...
  return Boolean(
    button && (button.pressed || button.value >= BUTTON_THRESHOLD),
  );
}

export function buttonsFromGamepad(
  gamepad: GamepadSnapshot,
  directionSource: GamepadDirectionSource,
): FrameButtons {
...
export function mergeButtons(
  ...sources: readonly FrameButtons[]
): FrameButtons {
  const merged = makeEmptyButtons();
  for (const action of ACTIONS)
    merged[action] = sources.some((source) => source[action]);
  return merged;
}
...
export function latchNewButtons(
  previous: FrameButtons,
  current: FrameButtons,
  latched: FrameButtons,
): FrameButtons {
  const next = { ...latched };
  for (const action of ACTIONS) {
...
export function isGamepadDirectionSource(
  value: unknown,
): value is GamepadDirectionSource {
  return (
    typeof value === "string" &&
    GAMEPAD_DIRECTION_SOURCES.includes(value as GamepadDirectionSource)
  );
}
