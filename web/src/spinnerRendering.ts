...
const VANILLA_SPINNERS: Record<Exclude<SpinnerVariant, 'theme'>, VisualTheme['spinner']> = {
  blue: { foreground: 'danger/crystal/fg_blue', background: 'danger/crystal/bg_blue' },
  red: { foreground: 'danger/crystal/fg_red', background: 'danger/crystal/bg_red' },
  purple: { foreground: 'danger/crystal/fg_purple', background: 'danger/crystal/bg_purple' },
  rainbow: { foreground: 'danger/crystal/fg_white', background: 'danger/crystal/bg_white', rainbow: true },
}

export function resolveSpinnerStyle(theme: VisualTheme, variant?: string): VisualTheme['spinner'] {
...
export function spinnerCenter(entity: MapEntity): { x: number; y: number } {
  return {
    x: entity.bounds.x + entity.bounds.width * .5,
    y: entity.bounds.y + entity.bounds.height * .5,
  }
}
...
export function spinnerHue(position: { x: number; y: number }, frame: number): string {
  // Mirrors CrystalStaticSpinner.GetHue: 280 px loop, 50 units/second,
  // hue 0.4..0.8, saturation 0.4 and value 0.9.
  const loop = ((Math.hypot(position.x, position.y) + frame / 60 * 50) % 280 + 280) % 280 / 280
  const yoYo = loop <= .5 ? loop * 2 : (1 - loop) * 2
  const hue = .4 + yoYo * .4
  const quantizedHue = Math.round(hue * 48) / 48
  return hsvToHex(quantizedHue, .4, .9)
}
