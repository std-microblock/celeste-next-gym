...
export class FrameCache {
  ...
  getSimulationInputs(count = this.buttons.length): SimInput[] {
    return this.buttons.slice(0, count).map((buttons, index) => ({
      ...(this.inputOverrides[index] ??
        buttonsToInput(
          buttons,
          index > 0 ? this.buttons[index - 1] : undefined,
        )),
    }));
  }
  ...
  getNearestState(frame: number): { frame: number; state: SimState } {
    for (
      let cursor = Math.min(frame, this.states.length - 1);
      cursor >= 0;
      cursor -= 1
    ) {
  ...
  ensureCapacity(requiredFrame: number): number {
    ...
    this.buttons = [
      ...this.buttons,
      ...Array.from({ length: added }, makeEmptyButtons),
    ];
    this.states = [
      ...this.states,
      ...Array.from<SimState | undefined>({ length: added }).fill(undefined),
    ];
    this.inputOverrides = [
      ...this.inputOverrides,
      ...Array.from<SimInput | undefined>({ length: added }).fill(undefined),
    ];
    this.emit();
    return size;
  }
  ...
  moveRun(
    ...
    const runStart = Math.max(
      0,
      Math.min(this.buttons.length, Math.round(start)),
    );
    const runEnd = Math.max(
      runStart,
      Math.min(this.buttons.length, Math.round(end)),
    );
    const length = runEnd - runStart;
    if (length === 0) return { start: runStart, end: runEnd };
    const targetStart = Math.max(
      0,
      Math.min(this.buttons.length - length, runStart + Math.round(delta)),
    );
    const targetEnd = targetStart + length;
    if (targetStart === runStart && targetAction === action)
      return { start: runStart, end: runEnd };
    const next = this.buttons.slice();
    for (let frame = runStart; frame < runEnd; frame += 1)
      next[frame] = { ...next[frame], [action]: false };
    for (let frame = targetStart; frame < targetEnd; frame += 1)
      next[frame] = { ...next[frame], [targetAction]: true };
    this.buttons = next;
    this.clearInputOverrides();
  ...
  replace(map: GymMap, initial: SimState, inputs: FrameButtons[]): void {
    this.map = map;
    this.buttons = inputs.length
      ? inputs.map((input) => ({ ...makeEmptyButtons(), ...input }))
      : [makeEmptyButtons()];
  ...
  private invalidateAfterInput(frame: number): void {
    this.states = this.states.map((state, stateFrame) =>
      stateFrame > frame ? undefined : state,
    );
    this.revision += 1;
    this.emit();
  }
  ...
  ensureFrame(target: number): Promise<SimState | undefined> {
    ...
    const task = this.queue.then(async () => {
      for (;;) {
        ...
        if (!checkpoint)
          throw new Error(`找不到帧 ${currentTarget} 的有效检查点`);
        const revision = this.revision;
        const inputs = this.buttons
          .slice(start, currentTarget)
          .map((buttons, offset) => {
            const absolute = start + offset;
            return (
              this.inputOverrides[absolute] ??
              buttonsToInput(
                buttons,
                absolute > 0 ? this.buttons[absolute - 1] : undefined,
              )
            );
          });
        const trace = await this.runner.simulate(checkpoint, inputs, this.map);
        if (revision !== this.revision) continue;
        if (trace.length !== inputs.length + 1)
          throw new Error(
            `WASM 轨迹长度错误：期望 ${inputs.length + 1}，实际 ${trace.length}`,
          );
        const states = this.states.slice();
        for (let offset = 1; offset < trace.length; offset += 1)
          states[start + offset] = trace[offset];
        this.states = states;
        this.emit();
    ...
    void task.then(
      () => {
        if (this.pendingFrames.get(clamped) === task)
          this.pendingFrames.delete(clamped);
      },
      () => {
        if (this.pendingFrames.get(clamped) === task)
          this.pendingFrames.delete(clamped);
