...
export function validateProcessIdentity(
  ...
  if (!actual)
    return {
      owned: false,
      reason: "process no longer exists or identity is unavailable",
    };
  if (actual.processId !== expected.processId) {
    return {
      owned: false,
      reason: `PID mismatch: expected ${expected.processId}, got ${actual.processId}`,
    };
  }
  if (
    comparablePath(actual.executablePath) !==
    comparablePath(expected.executablePath)
  ) {
    return {
      owned: false,
      reason: `executable mismatch: expected ${expected.executablePath}, got ${actual.executablePath}`,
    };
  }
  if (actual.creationTimeUtc !== expected.creationTimeUtc) {
    return {
      owned: false,
      reason: `creation time mismatch: expected ${expected.creationTimeUtc}, got ${actual.creationTimeUtc}`,
    };
  }
  return { owned: true };
}

export function queryProcessIdentity(
  processId: number,
...
`;
    ...
    if (result.status !== 0)
      throw new Error(`process identity query failed: ${result.stderr.trim()}`);
...
export async function waitForProcessIdentity(
  ...
  do {
    ...
    if (identity) {
      if (
        comparablePath(identity.executablePath) !==
        comparablePath(executablePath)
      ) {
        throw new Error(
          `spawned PID ${processId} executable mismatch: expected ${executablePath}, got ${identity.executablePath}`,
        );
      }
      return identity;
    }
  ...
  throw new Error(
    `spawned PID ${processId} identity was unavailable within ${timeoutMs} ms`,
  );
}
...
export async function terminateOwnedProcess(options: {
  ...
  try {
    ...
    (options.logger ?? console).warn(
      `refusing to terminate PID ${child.pid}: identity query failed: ${String(error)}`,
    );
    return false;
  }
  const decision = validateProcessIdentity(
    options.expectedIdentity,
    actualIdentity,
  );
  if (!decision.owned) {
    (options.logger ?? console).warn(
      `refusing to terminate PID ${child.pid}: ${decision.reason}`,
    );
    return false;
  }
...
async function waitForOwnedProcessExit(
  ...
  throw new Error(
    `owned PID ${expected.processId} did not exit within ${timeoutMs} ms`,
  );
}
