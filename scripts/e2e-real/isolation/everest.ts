...
export async function pingEverest(
  port: number,
  timeoutMs = 2_000,
): Promise<EverestPing> {
  return await new Promise<EverestPing>((resolvePing, reject) => {
    ...
    const timer = setTimeout(
      () =>
        finish(() => reject(new Error(`Everest ping timed out on ${port}`))),
      timeoutMs,
    );
    timer.unref();
    ...
    socket.once("end", () =>
      finish(() => reject(new Error("Everest closed ping without a response"))),
    );
  });
}
...
export function validateCollectorOwnership(
  ...
  if (response.run_nonce !== expected.runNonce) {
    throw new Error(
      `Everest run nonce mismatch: expected ${expected.runNonce}, got ${response.run_nonce ?? "(missing)"}`,
    );
  }
  if (response.process_id !== expected.processId) {
    throw new Error(
      `Everest process mismatch: expected ${expected.processId}, got ${response.process_id ?? "(missing)"}`,
    );
  }
  if (response.collector_port !== expected.port) {
    throw new Error(
      `Everest collector port mismatch: expected ${expected.port}, got ${response.collector_port ?? "(missing)"}`,
    );
  }
  if (response.success !== true)
    throw new Error(
      `Everest ping failed: ${response.error ?? "unknown error"}`,
    );
  return response;
}
...
export async function waitForOwnedEverest(
  ...
  do {
    try {
      ...
      if (
        /nonce mismatch|process mismatch|collector port mismatch/.test(message)
      )
        throw error;
      lastError = message;
    }
  ...
  throw new Error(
    `owned Everest collector was not ready within ${timeoutMs} ms: ${lastError}`,
  );
}
