...
function assertNoLinkedSegments(base: string, target: string): void {
  ...
  for (const segment of rel.split(sep).filter(Boolean)) {
    ...
    if (lstatSync(current).isSymbolicLink()) {
      throw new Error(
        `reparse/symlink path is not allowed for the E2E game install: ${current}`,
      );
    }
  }
}
...
export function validateGameInstall(options: {
  ...
  if (comparablePath(options.gameRoot) !== comparablePath(expectedRoot)) {
    throw new Error(
      `E2E game root must be ${expectedRoot}, got ${resolve(options.gameRoot)}`,
    );
  }
  ...
  if (comparablePath(realGameRoot) !== comparablePath(expectedRealRoot)) {
    throw new Error(
      `E2E game root resolves outside the repository vendor install: ${realGameRoot}`,
    );
  }
  ...
  if (
    comparablePath(dirname(realExecutable)) !== comparablePath(realGameRoot)
  ) {
    throw new Error(
      `Celeste executable resolves outside the isolated game root: ${realExecutable}`,
    );
  }
  ...
  if (/\/steamapps\/common\/celeste(?:\/|$)/i.test(normalizedGameRoot)) {
    throw new Error(
      `refusing to use a Steam Celeste install for E2E: ${realGameRoot}`,
    );
  }
  for (const steamRoot of options.steamRoots ?? []) {
    ...
    if (normalizedGameRoot === comparablePath(resolvedSteamRoot)) {
      throw new Error(
        `refusing to use configured Steam Celeste install for E2E: ${realGameRoot}`,
      );
    }
  }
