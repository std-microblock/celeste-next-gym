...
export function defineMapPart<const T extends MapPart>(part: T): T {
  if (!/^[a-z0-9][a-z0-9.-]*$/.test(part.id))
    throw new Error(`invalid map part id: ${part.id}`);
...
export function dependencyClosure(
  ...
  for (const part of [...requested].sort((left, right) =>
...
export function assembleFixturePackage(
  ...
  if (parts.length === 0)
    throw new Error("cannot assemble an empty map-part set");
  const packageName = parts[0]?.package;
  const sid = parts[0]?.sid;
  if (!packageName || !sid)
    throw new Error("map part package and sid must not be empty");
  const rooms = new Map<
    string,
    {
      bounds?: Rect;
  ...
  for (const part of parts) {
    if (part.package !== packageName || part.sid !== sid)
      throw new Error(
        `${part.id}: package or sid conflicts with dependency closure`,
      );
    for (const contribution of part.rooms) {
      for (const entity of contribution.entities ?? []) {
        ...
        if (previousRoom)
          throw new Error(
            `${part.id}: duplicate map-global fixture entity id ${entity.id} in ${previousRoom} and ${contribution.name}`,
          );
  ...
  const fixture: FixturePackage = {
    ...
    rooms: [...rooms.entries()]
      ...
      .map(([name, room]) => {
        if (!room.bounds || !room.spawn)
          throw new Error(`room ${name} is missing bounds or spawn`);
        const additionalSpawns = [...room.additionalSpawns.values()].sort(
        ...
        return {
          ...
          solids: [...room.solids.values()].sort((left, right) =>
...
function mergeRoom(
  ...
  const room = rooms.get(contribution.name) ?? {
    additionalSpawns: new Map<string, Vector2>(),
    solids: new Map<string, Rect>(),
    entities: new Map<string, FixtureEntity>(),
  };
  if (contribution.bounds)
    room.bounds = mergeScalar(
      `${partId}:${contribution.name}:bounds`,
      room.bounds,
      contribution.bounds,
    );
  if (contribution.spawn)
    room.spawn = mergeScalar(
      `${partId}:${contribution.name}:spawn`,
      room.spawn,
      contribution.spawn,
    );
  for (const spawn of contribution.additionalSpawns ?? [])
    room.additionalSpawns.set(stableJson(spawn), spawn);
  for (const solid of contribution.solids ?? [])
    room.solids.set(stableJson(solid), solid);
  for (const entity of contribution.entities ?? []) {
    validateAuthoringEntity(entity, contribution.name);
    const previous = room.entities.get(entity.id);
    if (previous && stableJson(previous) !== stableJson(entity))
      throw new Error(`${partId}: conflicting fixture entity ${entity.id}`);
...
function mergeScalar<T>(label: string, previous: T | undefined, next: T): T {
  if (previous !== undefined && stableJson(previous) !== stableJson(next))
...
export function validateFixturePackage(fixture: FixturePackage): void {
  if (fixture.formatVersion !== 1)
    throw new Error("fixture formatVersion must be 1");
  if (!fixture.package || !fixture.sid)
    throw new Error("fixture package and sid must not be empty");
  const roomNames = new Set<string>();
  const entityIds = new Set<string>();
  for (const room of fixture.rooms) {
    if (!room.name || roomNames.has(room.name))
      throw new Error(`duplicate or empty fixture room name: ${room.name}`);
    ...
    for (const [index, spawn] of (room.additionalSpawns ?? []).entries()) {
      ...
      assertPointInRoom(
        spawn,
        room.bounds,
        `room ${room.name} additionalSpawns[${index}]`,
      );
      if (stableJson(spawn) === stableJson(room.spawn))
        throw new Error(
          `room ${room.name} additionalSpawns[${index}] duplicates spawn`,
        );
    }
    ...
    for (const entity of room.entities) {
      ...
      if (entityIds.has(entity.id))
        throw new Error(`duplicate map-global fixture entity id: ${entity.id}`);
...
function validateAuthoringEntity(
  ...
  if (entity.kind === "fly_feather") {
    allowed.add("shielded");
  ...
  if (
    entity.kind === "badeline_boost" ||
    entity.kind === "zip_mover" ||
    entity.kind === "lookout"
  )
    allowed.add("nodes");
  ...
  if (
    ...
    entity.kind === "cassette_block"
  )
    allowed.add("direction");
  for (const key of Object.keys(entity))
    if (!allowed.has(key))
      throw new Error(
        `entity ${entity.id} kind ${entity.kind} forbids field ${key}`,
      );
  ...
  if ("direction" in entity) {
    ...
    if (entity.kind === "spikes" || entity.kind === "spring") {
      if (Math.abs(x) + Math.abs(y) !== 1)
        throw new Error(
          `entity ${entity.id} requires a cardinal unit direction`,
        );
    } else if (entity.kind === "wind" && (x === 0) === (y === 0)) {
      throw new Error(
        `entity ${entity.id} wind direction must have exactly one nonzero axis`,
      );
    } else if (
      (entity.kind === "move_block" || entity.kind === "moving_solid") &&
      x === 0 &&
      y === 0
    ) {
      throw new Error(
        `entity ${entity.id} ${entity.kind} direction must be nonzero`,
      );
    } else if (
      entity.kind === "cassette_block" &&
      (!Number.isInteger(x) || x < 0 || x > 3 || !Number.isInteger(y) || y <= 0)
    ) {
      throw new Error(
        `entity ${entity.id} cassette_block direction metadata must be [index 0..3, positive integer tempo]`,
      );
    }
  }
  if ("nodes" in entity && entity.nodes) {
    for (const [index, node] of entity.nodes.entries())
      assertVector(node, `entity ${entity.id} node ${index}`);
    if (entity.kind === "zip_mover" && entity.nodes.length !== 1)
      throw new Error(
        `entity ${entity.id} zip_mover requires exactly one node`,
      );
    if (entity.kind === "ice_ball" && entity.nodes.length > 1)
      throw new Error(`entity ${entity.id} ice_ball accepts at most one node`);
  }
  if ("shielded" in entity && typeof entity.shielded !== "boolean")
    throw new Error(`entity ${entity.id} shielded must be boolean`);
  if ("singleUse" in entity && typeof entity.singleUse !== "boolean")
    throw new Error(`entity ${entity.id} singleUse must be boolean`);
}
...
function validateCanonicalEntity(
  entity: CanonicalFixtureEntity,
  roomName: string,
): void {
  ...
  if (stableJson(keys) !== stableJson(expectedKeys))
    throw new Error(
      `entity ${entity.id} does not use the canonical entity schema`,
    );
  if (!entity.id)
    throw new Error(`room ${roomName} has an entity with an empty id`);
  if (!ENTITY_KINDS.has(entity.kind))
  ...
  for (const [index, node] of entity.nodes.entries())
    assertVector(node, `entity ${entity.id} node ${index}`);
  if (
    typeof entity.shielded !== "boolean" ||
    typeof entity.singleUse !== "boolean"
  ) {
  ...
  if (entity.kind === "spikes" || entity.kind === "spring") {
    if (Math.abs(x) + Math.abs(y) !== 1)
      throw new Error(`entity ${entity.id} requires a cardinal unit direction`);
    ...
    if (
      ...
      throw new Error(`entity ${entity.id} cassette metadata is invalid`);
  } else if (
    entity.kind !== "move_block" &&
    entity.kind !== "moving_solid" &&
    entity.kind !== "lookout" &&
    ...
    throw new Error(
      `entity ${entity.id} kind ${entity.kind} forbids nonzero direction`,
    );
  }
  if (entity.kind === "zip_mover" && entity.nodes.length !== 1)
    throw new Error(`entity ${entity.id} zip_mover requires exactly one node`);
  if (entity.kind === "ice_ball" && entity.nodes.length > 1)
    throw new Error(`entity ${entity.id} ice_ball accepts at most one node`);
  if (
  ...
  if (entity.shielded && entity.kind !== "fly_feather")
    throw new Error(`entity ${entity.id} kind ${entity.kind} forbids shielded`);
  if (
    entity.singleUse &&
...
function assertRect(rect: Rect, label: string, aligned: boolean): void {
  if (rect.length !== 4 || !rect.every(Number.isSafeInteger))
    throw new Error(`${label} must contain four integers`);
  if (rect[2] <= 0 || rect[3] <= 0)
    throw new Error(`${label} width and height must be positive`);
  if (aligned && !rect.every((value) => value % 8 === 0))
    throw new Error(`${label} must be 8px aligned`);
}

function assertVector(vector: Vector2, label: string): void {
  if (vector.length !== 2 || !vector.every(Number.isSafeInteger))
    throw new Error(`${label} must contain two integers`);
}
...
function assertRectInRoom(rect: Rect, room: Rect, label: string): void {
  ...
  if (
    x < roomX ||
    y < roomY ||
    x + width > roomX + roomWidth ||
    y + height > roomY + roomHeight
  ) {
