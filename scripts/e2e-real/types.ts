...
export type PlayerStateName =
  | 'Normal' | 'Climb' | 'Dash' | 'Swim' | 'Boost' | 'RedDash' | 'HitSquash'
  | 'Launch' | 'Pickup' | 'DreamDash' | 'SummitLaunch' | 'Dummy' | 'Frozen'
  | 'ReflectionFall' | 'StarFly' | 'TempleFall'
...
export type FixtureEntity =
  | (FixtureEntityBase & { readonly kind: 'jump_thru' | 'water' | 'dream_block' | 'bumper' | 'puffer' | 'angry_oshiro' | 'seeker' | 'snowball' | 'cloud' | 'bounce_block' | 'theo_crystal' | 'heart_gem' | 'sandwich_lava' | 'glider' | 'strawberry' | 'booster' | 'red_booster' | 'temple_gate' })
  | (FixtureEntityBase & { readonly kind: 'rising_lava'; readonly singleUse?: boolean })
...
export interface VerifyContext {
  ...
  near(actual: number | undefined, expected: number, tolerance?: number): boolean
