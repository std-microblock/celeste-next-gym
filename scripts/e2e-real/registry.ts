import type { MapPart, ScenarioDefinition, ScenarioStatus, TargetId } from './types.js'
...
export function buildRegistry(
  ...
  const implementedTechniqueIds = options.implementedTechniqueIds ?? new Set(
    definitions.filter((scenario) => scenario.status === 'active').flatMap((scenario) => scenario.techniqueIds),
  )
  const primaryByTechnique = new Map<string, string>()

  for (const scenario of definitions) {
    if (byName.has(scenario.name)) throw new Error(`duplicate scenario name: ${scenario.name}`)
    byName.set(scenario.name, scenario)
    validateScenarioMetadata(scenario, implementedTechniqueIds, primaryByTechnique)
    ...
    for (const part of scenario.mapParts) {
      ...
      if (previous && previous !== part && JSON.stringify(previous) !== JSON.stringify(part)) {
  ...
  const scenarios = Object.freeze([...definitions].sort((a, b) => a.name.localeCompare(b.name)))
  const byTarget = new Map<TargetId, readonly ScenarioDefinition[]>(
    [...byTargetMutable].map(([target, values]) => [target, Object.freeze([...values].sort((a, b) => a.name.localeCompare(b.name)))]),
  )
  return Object.freeze({ scenarios, byName, byTarget, counts: Object.freeze(counts), mapParts })
}
...
function validateScenarioMetadata(
  ...
  for (const techniqueId of recording.primaryFor) {
    ...
    if (scenario.status === 'candidate') {
      throw new Error(`${scenario.name}: candidate cannot be primary recording for implemented technique ${techniqueId}`)
    }
    const previous = primaryByTechnique.get(techniqueId)
    if (previous) throw new Error(`implemented technique ${techniqueId} has multiple primary recordings: ${previous}, ${scenario.name}`)
...
export function selectScenarios(
  ...
  if (options.names) {
    const missing = [...options.names].filter((name) => !registry.byName.has(name))
    if (missing.length > 0) throw new Error(`unknown E2E scenarios: ${missing.sort().join(', ')}`)
  }
  const selected = registry.scenarios.filter((scenario) => {
    ...
    if (!options.includeCandidates && scenario.status === 'candidate') return false
    if (options.disabledTags && [...options.disabledTags].some((tag) => scenario.tags.includes(tag))) return false
    return true
  })
  if (options.names) {
    const rejected = [...options.names].filter((name) => !selected.some((scenario) => scenario.name === name))
    if (rejected.length > 0) {
      throw new Error(`requested scenarios are excluded by target, candidate, or feature filters: ${rejected.sort().join(', ')}`)
    }
  }
