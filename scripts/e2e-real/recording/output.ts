...
export async function encodeScenarioArtifacts(
  options: {
  ...
  const master = await encode({
    ...
    outputPath: path.resolve(options.manifestPath, "..", "master.mp4"),
    ...media,
  });
  const result: RecordingArtifactEntry[] = [
    await makeEntry({
      recordingRoot: options.recordingRoot,
      kind: "scenario-master",
      scenarioId: options.item.scenario.name,
      stateSpan: {
        start_state_index: 0,
        end_state_index: options.item.scenario.inputs.length,
      },
      rawManifestPath: options.manifestPath,
  ...
  for (const techniqueId of options.item.techniqueIds) {
    ...
    const clip = await encode({
      ...
      outputPath: path.resolve(
        techniqueDirectory,
        `${options.item.scenario.name}.mp4`,
      ),
      posterPath: path.resolve(
        techniqueDirectory,
        `${options.item.scenario.name}.poster.png`,
      ),
      stateWindow: {
        startStateIndex: stateWindow.start_state_index,
        endStateIndex: stateWindow.end_state_index,
      },
...
export function resolveRecordingWindow(scenario: ScenarioDefinition): {
  ...
  if (recording?.startFrame !== undefined) {
    return {
      start_state_index: recording.startFrame,
      end_state_index: Math.min(recording.endFrame, scenario.inputs.length),
    };
  }
