...
export async function loadPresentationManifest(
  ...
  for (const frame of manifest.frames) {
    ...
    if (
      !info.isFile() ||
      info.size !== RAW_FRAME_BYTES ||
      frame.bytes !== RAW_FRAME_BYTES
    ) {
      throw new Error(
        `frame ${frame.render_index} is not a ${RAW_FRAME_BYTES}-byte BGRA file`,
      );
    }
...
export function validateManifest(value: unknown): PresentationManifest {
  ...
  if (
    ...
    root.capture_semantics !== "presentation_frames"
  ) {
    throw new Error("unsupported presentation manifest schema");
  }
  if (
    root.width !== RAW_WIDTH ||
    root.height !== RAW_HEIGHT ||
    root.pixel_format !== RAW_PIXEL_FORMAT
  ) {
    throw new Error("manifest does not describe 320x180 BGRA frames");
  }
  if (root.encoding_frame_rate !== 60)
    throw new Error("encoding_frame_rate must be 60");
  ...
  const frames = root.frames.map((value, index) => {
    ...
    if (frame.render_index !== index)
      throw new Error(`frames[${index}] render_index is not sequential`);
    const stateIndex = requireUnsignedInteger(
      frame.state_index,
      `frames[${index}].state_index`,
    );
    const timestamp = requireUnsignedInteger(
      frame.timestamp_ns,
      `frames[${index}].timestamp_ns`,
    );
    if (stateIndex < previousState)
      throw new Error(`frames[${index}] state_index moved backwards`);
    if (timestamp < previousTimestamp)
      throw new Error(`frames[${index}] timestamp moved backwards`);
    ...
    if (path.isAbsolute(relativePath))
      throw new Error(`frames[${index}].path must be relative`);
...
export async function canonicalDirectory(directory: string): Promise<string> {
  if (!path.isAbsolute(directory))
    throw new Error("recording root must be absolute");
