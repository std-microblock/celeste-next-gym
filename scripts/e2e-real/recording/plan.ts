...
const TARGET_ORDER: readonly TargetId[] = [
  "playground",
  "area-1",
  "area-2",
  "area-4",
...
export function createRecordingPlan(
  ...
  if (!config.recording)
    throw new Error("recording plan requires an explicit recording mode");
  const planned = new Map<
    string,
    { scenario: ScenarioDefinition; techniqueIds: Set<string> }
  >();
  ...
  if (config.recording.kind === "scenario") {
    ...
    const [scenario] = selectScenarios(registry, {
      names: new Set([name]),
      ...(config.targetExplicit && config.target
    ...
    for (const techniqueId of requestedTechniques) {
      ...
      if (technique.status !== "implemented")
        throw new Error(
          `technique ${techniqueId} is unimplemented and cannot be recorded`,
        );
      const primaries = registry.scenarios.filter((scenario) =>
        scenario.recording?.primaryFor.includes(techniqueId),
      );
      if (primaries.length !== 1)
        throw new Error(
          `implemented technique ${techniqueId} requires exactly one primary scenario, found ${primaries.length}`,
        );
      const scenario = primaries[0];
      if (!scenario || scenario.status !== "active")
        throw new Error(
          `implemented technique ${techniqueId} primary is not active`,
        );
      if (
        config.targetExplicit &&
        config.target &&
        scenario.target.id !== config.target
      ) {
        throw new Error(
          `technique ${techniqueId} primary target ${scenario.target.id} conflicts with ${config.target}`,
        );
      }
      if ([...config.disabledTags].some((tag) => scenario.tags.includes(tag))) {
        throw new Error(
          `technique ${techniqueId} primary ${scenario.name} is disabled by a feature filter`,
        );
      }
      const item = planned.get(scenario.name) ?? {
        scenario,
  ...
  const groups = TARGET_ORDER.flatMap((targetId) => {
    ...
    return [
      Object.freeze({
        target: scenarios[0]!.scenario.target,
  ...
  return Object.freeze({
    ...
    techniqueCount: new Set(
      [...planned.values()].flatMap((item) => [...item.techniqueIds]),
    ).size,
  });
}
