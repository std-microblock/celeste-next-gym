...
import { canonicalDirectory, ensurePathInside, type StateIndexRange } from './manifest.js'
...
export interface FileIdentity { readonly path: string; readonly sha256: string; readonly bytes: number }
...
export async function createArtifactEntry(options: {
  ...
  const poster = options.encoding.poster ? await identifyFile(root, options.encoding.poster.output_path) : undefined
  return {
    kind: options.kind,
...
async function identifyFile(root: string, candidate: string): Promise<FileIdentity> {
  const canonical = await realpath(candidate)
  ensurePathInside(root, canonical, 'artifact')
  const info = await stat(canonical)
  if (!info.isFile()) throw new Error(`artifact is not a regular file: ${canonical}`)
