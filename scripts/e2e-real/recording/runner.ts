...
export async function captureScenario<T>(options: {
  ...
  const captureToken = (options.createToken ?? (() => randomBytes(32).toString('base64url')))()
  ...
  if (started.state !== 'active') throw new Error(`recording start returned ${started.state}`)

  let execution: T
  try {
    execution = await options.execute(captureToken)
  } catch (error) {
    await stopAndFinalize(options.service, control, 'scenario execution failed').catch(() => undefined)
    throw error
  }
  ...
  if (status.state === 'active') {
    status = await options.service.recordingStop({ ...control, reason: 'runner poll timeout' })
    assertStatusIdentity(status, options.scenarioId, 0, options.endStateIndex)
  }
  ...
  if (finalized.state !== 'finalized' || !finalized.manifest_path) {
    throw new Error(`recording finalize returned ${finalized.state} without manifest_path`)
  }
  const presentation = await loadPresentationManifest(options.recordingRoot, finalized.manifest_path)
  const manifest = presentation.manifest
  if (manifest.scenario_id !== options.scenarioId) throw new Error('recording manifest scenario mismatch')
  if (manifest.run_nonce !== options.runNonce) throw new Error('recording manifest run nonce mismatch')
  if (manifest.process_id !== options.gameProcessId) throw new Error('recording manifest game process mismatch')
  const tokenHash = createHash('sha256').update(captureToken).digest('hex')
  if (manifest.capture_token_sha256 !== tokenHash) throw new Error('recording manifest capture token mismatch')
  if (manifest.start_state_index !== 0 || manifest.end_state_index !== options.endStateIndex) {
...
async function stopAndFinalize(service: RecordingService, request: RecordingControlRequest, reason: string): Promise<void> {
  try {
...
function assertStatusIdentity(status: RecordingStatus, scenarioId: string, start: number, end: number): void {
  if (status.scenario_id !== scenarioId || status.start_state_index !== start || status.end_state_index !== end) {
    throw new Error('recording service returned mismatched scenario or state span')
  }
}
