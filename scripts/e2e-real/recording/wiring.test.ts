...
describe('recording planning and lifecycle orchestration', () => {
  ...
  it('validates all 83 handbook primaries before running one isolated lifecycle per target', async () => {
    ...
    assert.equal(plan.techniqueCount, 83)
    assert.ok(plan.scenarioCount <= 83)
    ...
    assert.equal(summary.techniqueCount, 83)
  })
