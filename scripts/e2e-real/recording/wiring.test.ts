...
describe("recording planning and lifecycle orchestration", () => {
  it("passes the authenticated game ownership into the Everest collector backend", () => {
    ...
    assert.throws(
      () => collectorOwnershipEnvironment("nonce-1", 0),
      /positive process id/,
    );
  });
  ...
  it("requires explicit opt-in for candidate master captures while deduplicating multi-primary scenarios", () => {
    ...
    assert.throws(
      () => createRecordingPlan(unknown, registry, catalog),
      /unknown technique/,
    );
    const unimplementedId = catalog.techniques.find(
      (technique) => technique.status === "unimplemented",
    )!.id;
    const unimplemented = parseConfig(
      ["--record-tech", unimplementedId],
      {},
      repoRoot,
    );
    assert.throws(
      () => createRecordingPlan(unimplemented, registry, catalog),
      /unimplemented/,
    );
    const candidate = registry.scenarios.find(
      (scenario) => scenario.status === "candidate",
    )!;
    const candidateConfig = parseConfig(
      ["--record", "--scenario", candidate.name],
      {},
      repoRoot,
    );
    assert.throws(
      () => createRecordingPlan(candidateConfig, registry, catalog),
      /candidate|excluded/,
    );
    const candidatePlan = createRecordingPlan(
      parseConfig(
        ["--record", "--scenario", candidate.name, "--include-candidates"],
        {},
        repoRoot,
      ),
      registry,
    ...
    const shared = {
      ...base,
      name: "shared-primary-test",
      techniqueIds: ["a", "b"],
      recording: {
        primaryFor: ["a", "b"],
        startFrame: 0,
        endFrame: base.inputs.length,
      },
    } as typeof base;
    const sharedCatalog = {
      techniques: [
...
describe("recording service state machine", () => {
  it("uses one token for start and simulate, then finalizes and validates the owned raw manifest", async () => {
    ...
    const service = fakeService(
      path.relative(fixture.root, fixture.manifestPath),
      events,
      "ready",
    );
  ...
  it("rejects negative counters and reversed ranges in collector responses", () => {
    ...
    assert.throws(
      () => validateRecordingStatus({ ...valid, render_frame_count: -1 }),
      /negative/,
    );
    assert.throws(
      () =>
        validateRecordingStatus({
          ...valid,
          unpresented_update_ranges: [
            { start_state_index: 2, end_state_index: 1 },
          ],
        }),
      /reversed/,
    );
...
describe("recording outputs", () => {
  it("captures once but emits one windowed clip and poster per primary technique", async () => {
    ...
    const scenario = {
      ...
      name: "multi-output",
      techniqueIds: ["a", "b"],
      recording: { primaryFor: ["a", "b"], startFrame: 2, endFrame: 4 },
    } as typeof base;
    const calls: Array<{
      outputPath: string;
      posterPath?: string;
    ...
    const fakeResult: EncodingResult = {
      output_path: path.join(root, "fake.mp4"),
      sha256: "a".repeat(64),
      bytes: 1,
      probe: {
        codec: "h264",
    ...
    const artifacts = await encodeScenarioArtifacts(
      {
        item: { scenario, techniqueIds: ["a", "b"] },
        manifestPath: path.join(root, "raw", "manifest.json"),
        tracePath: path.join(root, "raw", "trace.json"),
        recordingRoot: root,
      ...
      },
      {
        ...
        createEntry: async (entry) =>
          ({
            ...entry,
            raw_manifest: {} as never,
            trace: {} as never,
            video: {} as never,
            media_processes: [],
          }) as unknown as RecordingArtifactEntry,
      },
    );
    ...
    for (const [index, technique] of ["a", "b"].entries()) {
      ...
      assert.deepEqual(call.stateWindow, {
        startStateIndex: 2,
        endStateIndex: 4,
      });
      assert.equal(call.posterStateIndex, undefined);
      assert.ok(
        call.outputPath.endsWith(
          path.join("techniques", technique, "multi-output.mp4"),
        ),
      );
      assert.ok(
        call.posterPath!.endsWith(
          path.join("techniques", technique, "multi-output.poster.png"),
        ),
      );
    }
  });
  ...
  it("passes an explicit poster state without changing the full recording window", async () => {
    ...
    assert.deepEqual(calls[1]!.stateWindow, {
      startStateIndex: 0,
      endStateIndex: base.inputs.length,
    });
  });
  ...
  it("rejects artifact paths outside the recording root and atomically writes the manifest", async () => {
    ...
    await assert.rejects(
      () =>
        createArtifactEntry({
          recordingRoot: root,
          kind: "scenario-master",
          scenarioId: "scenario-1",
          stateSpan: { start_state_index: 0, end_state_index: 1 },
          rawManifestPath: outsideFile,
          tracePath: outsideFile,
          encoding: {
    ...
    const manifestPath = await writeArtifactManifest({
      ...
      gameProcess: {
        ...
        creationTimeUtc: new Date(0).toISOString(),
      },
      artifacts: [],
    });
    assert.equal(
      JSON.parse(await readFile(manifestPath, "utf8")).schema_version,
      1,
    );
...
function fakeService(
  ...
  return {
    recordingStart: async (request: { capture_token: string }) => {
      events.push(`start:${request.capture_token}`);
    ...
    recordingStop: async (request: { reason?: string }) => {
      events.push(`stop:${request.reason}`);
...
async function createRawFixture(
  ...
  await Promise.all(
    presentationFrames.map(async (frame) => {
  ...
  const manifest: PresentationManifest = {
    ...
    finalized_at: new Date(1).toISOString(),
    outcome: "ready",
    start_state_index: 0,
    end_state_index: 1,
    latest_state_index: 1,
    final_state_presented: true,
    repeated_presentation_count: 60,
    unpresented_update_ranges: [],
