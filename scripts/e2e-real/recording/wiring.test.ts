...
describe('recording planning and lifecycle orchestration', () => {
  ...
  it('validates all 79 handbook primaries before running one isolated lifecycle per target', async () => {
    ...
    assert.equal(plan.techniqueCount, 79)
    assert.ok(plan.scenarioCount <= 79)
    ...
    assert.equal(summary.techniqueCount, 79)
  })
