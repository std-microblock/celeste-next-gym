...
describe('recording planning and lifecycle orchestration', () => {
  ...
  it('validates all 63 handbook primaries before running one isolated lifecycle per target', async () => {
    ...
    assert.equal(plan.techniqueCount, 63)
    assert.ok(plan.scenarioCount <= 63)
    ...
    assert.equal(summary.techniqueCount, 63)
  })
