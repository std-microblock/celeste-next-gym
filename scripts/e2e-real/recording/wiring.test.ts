...
describe('recording planning and lifecycle orchestration', () => {
  ...
  it('validates all 58 handbook primaries before running one isolated lifecycle per target', async () => {
    ...
    assert.equal(plan.techniqueCount, 58)
    assert.ok(plan.scenarioCount <= 58)
    ...
    assert.equal(summary.techniqueCount, 58)
  })
