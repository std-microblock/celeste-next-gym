...
describe('recording planning and lifecycle orchestration', () => {
  ...
  it('validates all 86 handbook primaries before running one isolated lifecycle per target', async () => {
    ...
    assert.equal(plan.techniqueCount, 86)
    assert.ok(plan.scenarioCount <= 86)
    ...
    assert.equal(summary.techniqueCount, 86)
  })
