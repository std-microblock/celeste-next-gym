...
describe('recording planning and lifecycle orchestration', () => {
  ...
  it('validates all 89 handbook primaries before running one isolated lifecycle per target', async () => {
    ...
    assert.equal(plan.techniqueCount, 89)
    assert.ok(plan.scenarioCount <= 89)
    ...
    assert.equal(summary.techniqueCount, 89)
  })
