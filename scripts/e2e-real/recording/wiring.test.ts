...
describe('recording planning and lifecycle orchestration', () => {
  ...
  it('validates all 78 handbook primaries before running one isolated lifecycle per target', async () => {
    ...
    assert.equal(plan.techniqueCount, 78)
    assert.ok(plan.scenarioCount <= 78)
    ...
    assert.equal(summary.techniqueCount, 78)
  })
