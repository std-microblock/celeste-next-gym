...
describe('authoritative technique recording metadata', () => {
  ...
  it('assigns exactly one primary to every implemented technique and none to unimplemented techniques', () => {
    ...
    assert.equal(techniques.filter((technique) => technique.status === 'implemented').length, 94)
    assert.equal(techniques.filter((technique) => technique.status === 'unimplemented').length, 26)
