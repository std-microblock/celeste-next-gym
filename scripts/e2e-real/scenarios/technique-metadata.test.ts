...
function techniqueCatalog(): TechniqueEvidence[] {
  return authoritativeTechniqueFiles().map((path) => {
    ...
    return {
      id,
      status,
      e2eSymbols: evidenceSymbols(
        text.match(
          /e2e-evidence:\s*evidence\(([\s\S]*?)\),\s*candidate-e2e:/,
        )?.[1],
      ),
      candidateSymbols: evidenceSymbols(
        text.match(/candidate-e2e:\s*evidence\(([\s\S]*?)\),?\s*\n\)/)?.[1],
      ),
    };
  });
}
...
describe("authoritative technique recording metadata", () => {
  ...
  it("assigns exactly one primary to every implemented technique and none to nonimplemented techniques", () => {
    ...
    assert.equal(
      techniques.filter((technique) => technique.status === "implemented")
        .length,
      114,
    );
    assert.equal(
      techniques.filter((technique) => technique.status === "unimplemented")
        .length,
      2,
    );
    assert.equal(
      techniques.filter((technique) => technique.status === "product-excluded")
        .length,
      4,
    );
    ...
    for (const scenario of scenarios) {
      for (const techniqueId of scenario.techniqueIds) {
        assert.ok(
          byId.has(techniqueId),
          `${scenario.name}: unknown technique id ${techniqueId}`,
        );
      }
      if (!scenario.recording) continue;
      assert.equal(
        scenario.recording.startFrame,
        0,
        `${scenario.name}: recording must preserve the full verified trace`,
      );
      assert.equal(
        scenario.recording.endFrame,
        scenario.inputs.length,
        `${scenario.name}: recording must preserve the full verified trace`,
      );
      for (const techniqueId of scenario.recording.primaryFor) {
        assert.ok(
          scenario.techniqueIds.includes(techniqueId),
          `${scenario.name}: primary is not a technique link`,
        );
        primaryCounts.set(
          techniqueId,
          (primaryCounts.get(techniqueId) ?? 0) + 1,
        );
      }
    }
    ...
    for (const technique of techniques) {
      assert.equal(
        primaryCounts.get(technique.id) ?? 0,
        technique.status === "implemented" ? 1 : 0,
        technique.id,
      );
    }
  });
    ...
    for (const technique of techniques) {
      const symbols =
        technique.status === "implemented"
          ? technique.e2eSymbols
          : technique.candidateSymbols;
      const evidenceScenarios = symbols
        .map((symbol) => byName.get(symbol))
        .filter((scenario) => scenario !== undefined);
      if (technique.status === "implemented")
        assert.ok(
          evidenceScenarios.length > 0,
          `${technique.id}: no E2E scenario symbol`,
        );
      for (const scenario of evidenceScenarios) {
        assert.ok(
          scenario.techniqueIds.includes(technique.id),
          `${technique.id}: missing link on ${scenario.name}`,
        );
      }
      const primaries = scenarios.filter((scenario) =>
        scenario.recording?.primaryFor.includes(technique.id),
      );
      if (technique.status === "implemented")
        assert.deepEqual(
          primaries.map((scenario) => scenario.name),
          [evidenceScenarios[0]?.name],
        );
      else
        assert.deepEqual(
          primaries,
          [],
          `${technique.id}: unimplemented technique has a primary`,
        );
    }
  });
});
