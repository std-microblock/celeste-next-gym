...
export const scenario = defineScenario({
  ...
  inputs: Array.from({ length: 150 }, (_, frame) =>
    input({
  ...
  verify(states) {
    ...
    semanticAssert(
      field<boolean>(states[1], "lookoutInteracting") === true,
      scenario.name,
      "Talk did not start the entity-owned Lookout interaction on frame one",
    );
    semanticAssert(
      states[2]?.state === 11 || states[2]?.state === "Dummy",
      scenario.name,
      "Lookout did not force Player.StDummy on frame two",
    );
    semanticAssert(
      cameras.some(
        (camera) => (camera?.[0] ?? 0) >= (cameras[0]?.[0] ?? 0) + 186,
      ),
      scenario.name,
      "Aim input did not move Level.Camera through the certified 186px range",
    );
    semanticAssert(
      (states[129]?.state === 11 || states[129]?.state === "Dummy") &&
        field<boolean>(states[129], "lookoutInteracting") === true,
      scenario.name,
      "LookRoutine did not retain Dummy interaction through frame 129",
    );
    semanticAssert(
      states[130]?.state === 0 &&
        field<boolean>(states[130], "lookoutInteracting") === false,
      scenario.name,
