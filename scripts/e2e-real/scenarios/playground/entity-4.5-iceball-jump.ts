...
export const scenario = defineScenario({
  ...
  recording: { primaryFor: ['4.5'], startFrame: 0, endFrame: 24 },
    ...
    verify(states) {
      const bounced = states.find((state) => Math.abs(state.speed[1] + 140) <= 0.01)
      if (states.some((state) => state.dead) || !bounced || bounced.state !== 0
        || bounced.speed[0] <= 160 || bounced.dashes !== 1 || Math.abs(bounced.stamina - 110) > 0.01) {
        throw new Error(`entity-4.5-iceball-jump: top bounce was ${JSON.stringify(bounced)}`)
      }
    },
})
