...
export const scenario = defineScenario({
  ...
  verify(states) {
    const exitIndex = states.findIndex(
      (state, index) =>
        index > 0 && states[index - 1]?.state === 19 && state.state === 0,
    );
    const exit = states[exitIndex];
    if (!exit || exit.pos[1] < 402 || exit.on_ground || exit.dead) {
      throw new Error(
        `entity-4.15.1-feather-clip: StarFly did not expire below the isolated 16px jumpthrough: ${JSON.stringify(
          {
            exit: pickCore(exit),
            neighbors: states
              .slice(Math.max(0, exitIndex - 2), exitIndex + 3)
              .map(pickCore),
          },
        )}`,
      );
