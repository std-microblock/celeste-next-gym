...
function verifySpikeClip(states: readonly E2EState[]): void {
  semanticAssert(
    ...
    "spike-clip",
    "high-speed fall was caught by the unsupported spike strip",
  );
  semanticAssert(
    near(states[1]?.pos[1], 107) && (states[1]?.speed[1] ?? 0) > 220,
    "spike-clip",
    `first crossing was pos=${states[1]?.pos[1]} speed=${states[1]?.speed[1]}`,
  );
  semanticAssert(
    (states[1]?.pos[1] ?? 0) - 2 > 103,
    "spike-clip",
    "hurtbox bottom did not cross below the three-pixel spike collider",
  );
}
