...
export const scenario = defineScenario({
  ...
  verify(states) {
    const landed = states.findIndex(
      (state, frame) => frame > 10 && state.on_ground && state.pos[1] <= 416,
    );
    const clipped = states.findIndex(
      (state, frame) => frame > landed && state.pos[1] > 416 && !state.dead,
    );
    semanticAssert(
      landed >= 0,
      scenario.name,
      "player never reached the JumpThru before the pusher",
    );
    semanticAssert(
      clipped > landed,
      scenario.name,
      `clip frame=${clipped}, landed=${landed}`,
    );
    semanticAssert(
      states.slice(0, clipped + 1).every((state) => !state.dead),
      scenario.name,
