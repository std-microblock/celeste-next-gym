...
export const scenario = defineScenario({
  ...
  inputs: Array.from({ length: 72 }, (_, frame) =>
    input({
      move_x: frame >= 24 && frame < 40 ? -1 : frame >= 40 ? 1 : 0,
      move_y: frame === 23 ? 1 : 0,
      dash_pressed: frame === 40,
...
function verifyJellyRegrab(states: readonly E2EState[]): void {
  const first = states.findIndex(
    (state) => state.state === 8 && state.holding_glider,
  );
  const released = states.findIndex(
    (state, frame) => frame > first && !state.holding_glider,
  );
  const regrab = states.findIndex(
    (state, frame) =>
      frame > released && state.state === 8 && state.holding_glider,
  );
  semanticAssert(
    first >= 0 && released > first,
    scenario.name,
    `pickup=${first}, release=${released}`,
  );
  semanticAssert(
    regrab > released && near(states[regrab]?.speed[0], 0),
    scenario.name,
    `regrab=${regrab}`,
  );
}
