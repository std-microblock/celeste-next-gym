...
function verifyDemodashGap(states: readonly E2EState[]): void {
  semanticAssert(
    states.some((state) => state.ducking && state.pos[0] > 720),
    "dash-demodash-gap",
    "crouched dash never entered the six-pixel tunnel",
  );
  const last = states.at(-1);
  semanticAssert(
    last && last.pos[0] > 760 && !last.dead,
    "dash-demodash-gap",
    "demo did not progress through the low tunnel",
  );
}
