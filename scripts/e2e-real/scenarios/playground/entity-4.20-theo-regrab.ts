...
function verifyTheoRegrab(states: readonly E2EState[]): void {
  ...
  semanticAssert(states.slice(released, Math.min(released + 6, states.length)).every((state) => !state.holding_theo),
    scenario.name, 'CannotHold did not block the immediate regrab')
  semanticAssert(regrabbed > released && dashBeforeRegrab?.state === 2 && near(dashBeforeRegrab.speed[0], 240),
    scenario.name, `regrab=${regrabbed}, before=${JSON.stringify(dashBeforeRegrab)}`)
}
