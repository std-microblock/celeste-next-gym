...
function verifyGroundedUltraCancelControl(states: readonly E2EState[]): void {
  semanticAssert(states.every((state) => state.state !== 8 && state.holding_theo !== true),
    'dash-grounded-ultra-cancel-control', 'control unexpectedly picked up Theo')
  semanticAssert(states.some((state) => state.state === 2 && near(state.speed[0], 360) && state.ducking),
    'dash-grounded-ultra-cancel-control', 'control never formed the 360 grounded Ultra')
  const normalized = states.find((state) => state.state === 0 && near(state.speed[0], 160))
  semanticAssert(normalized, 'dash-grounded-ultra-cancel-control',
