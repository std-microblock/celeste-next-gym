...
function verifyDashAimSampling(states: readonly E2EState[]): void {
  const entered = states.findIndex(
    (state, frame) =>
      frame > 0 && (state.state === "Dash" || state.state === 2),
  );
  semanticAssert(
    entered === 2,
    scenario.name,
    `Dash did not begin on state frame 2: ${JSON.stringify(states.slice(0, 4).map(pickCore))}`,
  );
  const beginDir = field<readonly number[]>(states[entered], "DashDir");
  semanticAssert(
    near(states[entered]?.speed[0], 0) &&
      near(states[entered]?.speed[1], 0) &&
      beginDir &&
      near(beginDir[0], 0) &&
      near(beginDir[1], 0),
    scenario.name,
  ...
  const launched = states.findIndex(
    (state, frame) =>
      frame > entered &&
      (state.state === "Dash" || state.state === 2) &&
      near(state.speed[0], -240) &&
  ...
  semanticAssert(
    launched === 6 &&
      launchedDir &&
      near(launchedDir[0], -1) &&
      near(launchedDir[1], 0) &&
      (states[launched]?.facing === false ||
        states[launched]?.facing === "Left"),
    scenario.name,
