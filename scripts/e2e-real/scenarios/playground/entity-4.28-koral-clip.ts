...
export const scenario = defineScenario({
  ...
  verify(states) {
    const gateStop = states.findIndex((state, frame) => frame > 8
      && state.on_ground
      && state.pos[0] <= 525
      && state.pos[0] >= 523)
    semanticAssert(states.every((state) => !state.dead), scenario.name, 'TempleGate close killed the player')
    semanticAssert(gateStop >= 0, scenario.name,
    ...
    const clippedTheo = states.find((state) => {
      ...
      return position?.length === 2 && (Math.abs(Number(position[0]) - 512) > 0.01
        || Math.abs(Number(position[1]) - 476) > 0.01)
    })
    semanticAssert(clippedTheo, scenario.name, 'TempleGate never displaced Theo from its source position')
  },
})
