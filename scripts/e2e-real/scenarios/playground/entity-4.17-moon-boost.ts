...
export const scenario = defineScenario({
  ...
  verify(states) {
    const diagonal = states.find((state) => {
      ...
      return (
        field(state, "reformBlockKind") === "MoveBlock" &&
        position?.length === 2 &&
    ...
    const boosted = states.find(
      (state) => state.state === 0 && state.speed[1] < -105.01,
    );
    semanticAssert(
      !states.some((state) => state.dead),
      scenario.name,
      "player died during the steered MoveBlock jump",
    );
    semanticAssert(
      diagonal,
      scenario.name,
      "MoveBlock never exposed an up-right steered position",
    );
    semanticAssert(
      boosted,
      scenario.name,
      "jump never inherited a negative vertical MoveBlock lift",
    );
  },
});
