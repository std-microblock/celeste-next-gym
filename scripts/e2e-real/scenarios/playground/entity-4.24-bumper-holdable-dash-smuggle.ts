...
export const scenario = defineScenario({
  ...
  verify(states) {
    ...
    const launch = states.findIndex(
      (state, frame) =>
        frame > firstPickup && state.state === 7 && !state.holding_theo,
    );
    const dash = states.findIndex(
      (state, frame) =>
        frame > launch && state.state === 2 && !state.holding_theo,
    );
    const regrab = states.findIndex(
      (state, frame) => frame > dash && state.state === 8 && state.holding_theo,
    );
    semanticAssert(
      firstPickup >= 0 && launch > firstPickup,
      scenario.name,
      `pickup=${firstPickup}, launch=${launch}`,
    );
    semanticAssert(
      dash > launch,
      scenario.name,
      `dash=${dash}, launch=${launch}`,
    );
    semanticAssert(
      regrab > dash,
      scenario.name,
