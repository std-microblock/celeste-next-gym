...
export const scenario = defineScenario({
  ...
  inputs: Array.from({ length: 42 }, (_, frame) =>
    input({
      move_x: frame < 23 ? 1 : -1,
      grab_held: frame <= 22,
    }),
  ...
  verify(states) {
    const pickup = states.findIndex(
      (state) => state.state === 8 && state.holding_theo,
    );
    const released = states.find(
      (state, frame) => frame > pickup && !state.holding_theo && !state.facing,
    );
    semanticAssert(
      pickup >= 0 && Boolean(released),
      scenario.name,
      `pickup=${pickup}, released=${JSON.stringify(released)}`,
    );
    semanticAssert(
      (released?.speed[0] ?? 0) > 120,
      scenario.name,
      `backboost speed=${released?.speed[0]}`,
    );
  },
});
