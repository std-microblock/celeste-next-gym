...
export const scenario = defineScenario({
  ...
  verify(states) {
    ...
    if (
      last?.state !== 0 ||
      Math.abs(last.speed[0] - 325) > 0.01 ||
      Math.abs(last.speed[1] + 52.5) > 0.01 ||
      last.dashes !== 1
    ) {
      throw new Error(
        "entity-4.2-bubble-demohyper: did not end on the expected 325/-52.5 demohyper with the booster dash retained",
      );
    }
  },
});
