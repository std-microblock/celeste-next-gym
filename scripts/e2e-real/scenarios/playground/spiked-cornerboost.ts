...
export const scenario = defineScenario({
  ...
  recording: { primaryFor: ["3.7.10"], startFrame: 0, endFrame: 20 },
...
function verifySpikedCornerboost(states: readonly E2EState[]): void {
  semanticAssert(
    ...
    "spiked-cornerboost",
    "upward cornerboost was killed by the top spikes while moving away from their points",
  );
  semanticAssert(
    near(states[1]?.stamina, 82.5) && near(states[1]?.speed[0], 0),
    "spiked-cornerboost",
    "cornerboost did not store its first horizontal collision",
  );
  semanticAssert(
    Number(field(states[1], "wallSpeedRetained")) > 120,
    "spiked-cornerboost",
    `retained speed was ${String(field(states[1], "wallSpeedRetained"))}`,
  );
}
