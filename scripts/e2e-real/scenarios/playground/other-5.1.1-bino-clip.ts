...
export const scenario = defineScenario({
  ...
  inputs: Array.from({ length: 230 }, (_, frame) =>
    input({
  ...
  verify(states) {
    ...
    const visible = states.map((state) =>
      field<boolean>(state, "crystalSpinnerVisible"),
    );
    const collidable = states.map((state) =>
      field<boolean>(state, "crystalSpinnerCollidable"),
    );
    semanticAssert(
      visible.some(Boolean),
      scenario.name,
      "spinner never entered its real camera view",
    );
    const offscreenFrame = cameras.findIndex(
      (camera) => (camera?.[0] ?? Infinity) < 300,
    );
    semanticAssert(
      offscreenFrame >= 0,
      scenario.name,
    ...
    const hiddenWithinInterval = visible
      .slice(offscreenFrame, offscreenFrame + 16)
      .some(
        (value, index) =>
          value === false && collidable[offscreenFrame + index] === false,
      );
    semanticAssert(
      hiddenWithinInterval,
      scenario.name,
