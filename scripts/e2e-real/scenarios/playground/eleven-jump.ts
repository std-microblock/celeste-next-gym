...
function verifyElevenJump(states: readonly E2EState[]): void {
  const completed = states.findIndex(
    (state, frame) =>
      frame > 0 && state.dashes >= 1 && near(state.stamina, 110),
  );
  semanticAssert(
    completed === 41,
    "eleven-jump",
    `room transition completed at state ${completed} instead of 41`,
  );
  semanticAssert(
    near(states[42]?.stamina, 82.5) &&
      near(states[43]?.stamina, 55) &&
      near(states[44]?.stamina, 27.5),
    "eleven-jump",
    "three buffered climb jumps did not execute immediately after the transition",
  );
  semanticAssert(
    Number(field(states[44], "wallSpeedRetained")) > 190,
    "eleven-jump",
    `retained speed was ${String(field(states[44], "wallSpeedRetained"))}`,
  );
  semanticAssert(
    states.some((state) => state.on_ground && state.pos[0] >= 412),
    "eleven-jump",
    "player did not land across the eleven-tile gap",
  );
}
