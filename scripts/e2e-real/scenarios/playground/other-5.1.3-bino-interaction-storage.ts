...
export const scenario = defineScenario({
  ...
  inputs: Array.from({ length: 300 }, (_, frame) => input({
    ...
    // Movement begins after the native Booster has interrupted the completed
    // Dummy walk and LookRoutine owns the camera independently of Player.
  ...
  verify(states) {
    ...
    semanticAssert(states.some((state) => field<boolean>(state, 'boosterBoostingPlayer') === true || state.state === 4), scenario.name,
      'the native Booster never interrupted the completed Lookout Dummy walk')
