...
export const scenario = defineScenario({
  ...
  inputs: Array.from({ length: 300 }, (_, frame) =>
    input({
  ...
  verify(states) {
    semanticAssert(
      states.some(
        (state) => field<boolean>(state, "lookoutInteracting") === true,
      ),
      scenario.name,
    ...
    semanticAssert(
      states.some(
        (state) =>
          field<boolean>(state, "boosterBoostingPlayer") === true ||
          state.state === 4,
      ),
      scenario.name,
      "the native Booster never interrupted the Lookout Dummy state",
    );
    const removedAt = states.findIndex(
      (state) => field<boolean>(state, "lookoutRemovalObserved") === true,
    );
    semanticAssert(
      removedAt >= 0,
      scenario.name,
    ...
    semanticAssert(
      field<boolean>(states[removedAt], "lookoutRemovedWhileInteracting") ===
        true,
      scenario.name,
      "Lookout.Removed did not observe its entity-owned interacting flag",
    );
    semanticAssert(
      field<number>(states[removedAt], "lookoutRemovalPlayerState") === 0 &&
        states[removedAt]?.state === 0,
      scenario.name,
      "room removal did not restore Player.StNormal while retaining the Lookout interaction",
    );
    semanticAssert(
      states.slice(removedAt).some((state) => state.pos[0] >= 960),
      scenario.name,
