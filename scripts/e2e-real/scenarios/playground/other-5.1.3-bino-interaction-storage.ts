...
export const scenario = defineScenario({
  ...
  status: 'candidate',
  tags: ['feature:lookout', 'feature:transition', 'storage:interaction-removal'],
  techniqueIds: ['5.1.3'],
  mapParts,
  name: 'other-5.1.3-bino-interaction-storage',
  initial: { pos: [944, 496], speed: [0, 0], on_ground: true },
  inputs: Array.from({ length: 130 }, (_, frame) => input({ talk_pressed: frame === 0 })),
  verify(states) {
    semanticAssert(states.some((state) => field<boolean>(state, 'lookoutInteracting') === true), scenario.name,
      'Talk did not reach Lookout.interacting before the boundary walk')
