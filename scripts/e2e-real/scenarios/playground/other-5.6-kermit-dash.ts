...
function verifyKermitDash(states: readonly E2EState[]): void {
  const entered = states.findIndex((state, frame) => frame > 0 && state.state === 'Normal' && near(state.speed[1], -105))
  ...
  const hit = states.findIndex((state, frame) => frame > entered && state.state === 'StarFly')
