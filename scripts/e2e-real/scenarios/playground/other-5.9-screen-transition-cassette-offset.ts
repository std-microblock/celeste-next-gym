...
export const scenario = defineScenario({
  ...
  verify(states) {
    ...
    const sharedPhase = states.findIndex((state, frame) => {
      ...
      return frame > entered && blocks.length === 4
        && blocks.some((block) => block.index === 0 && block.position[1] === 18 && !block.collidable)
        && blocks.some((block) => block.index === 0 && block.position[1] === -46 && !block.collidable)
        && blocks.some((block) => block.index === 1 && block.position[1] === 16 && block.collidable)
        && blocks.some((block) => block.index === 1 && block.position[1] === -48 && block.collidable)
    })
    ...
    semanticAssert(sharedPhase > entered && sharedPhase < completed, scenario.name,
