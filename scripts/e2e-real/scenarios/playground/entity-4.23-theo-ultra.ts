...
export const scenario = defineScenario({
  ...
  recording: { primaryFor: ["4.23"], startFrame: 0, endFrame: 24 },
  ...
  verify(states) {
    ...
    const restored = states.find(
      (state, frame) =>
        frame > 0 && states[frame - 1]?.state === 8 && state.state === 0,
    );
    semanticAssert(
      Boolean(pickup) && near(pickup?.speed[0], 0),
      scenario.name,
      "Theo pickup did not cancel the grounded ultra into Pickup",
    );
    semanticAssert(
      Boolean(restored) && near(restored?.speed[0], 360),
      scenario.name,
