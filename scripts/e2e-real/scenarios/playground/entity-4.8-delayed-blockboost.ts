...
export const scenario = defineScenario({
  ...
  inputs: Array.from({ length: 25 }, (_, frame) =>
    input({
      move_x: frame >= 8 ? 1 : 0,
  ...
  verify(states) {
    ...
    if (
      !before ||
      before.on_ground ||
      Math.abs(before.pos[0] - 108) > 0.01 ||
      jumped?.state !== 0 ||
      Math.abs(jumped.speed[0] + 130) > 0.01 ||
      Math.abs(jumped.speed[1] + 230.828) > 0.01
    ) {
      throw new Error(
        `entity-4.8-delayed-blockboost: did not apply retained ZipMover lift to the later static-wall jump: ${JSON.stringify(
          {
