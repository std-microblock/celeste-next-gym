...
function verifyNarrowSpikedClimb(states: readonly E2EState[]): void {
  semanticAssert(
    ...
    "narrow-spiked-climb",
    "alternating wall jumps died in the narrow spiked shaft",
  );
  semanticAssert(
    near(states[1]?.speed[0], -130) && near(states[4]?.speed[0], 130),
    "narrow-spiked-climb",
    `away speeds were ${states[1]?.speed[0]} and ${states[4]?.speed[0]}`,
  );
  semanticAssert(
    (states[5]?.pos[1] ?? 152) < 145,
    "narrow-spiked-climb",
    "the alternating jumps did not climb the shaft",
  );
}
