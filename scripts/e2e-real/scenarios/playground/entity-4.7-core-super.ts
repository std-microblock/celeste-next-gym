...
export const scenario = defineScenario({
  ...
  techniqueIds: ["4.7"],
  recording: { primaryFor: ["4.7"], startFrame: 0, endFrame: 38 },
  mapParts,
  name: "entity-4.7-core-super",
  initial: { pos: [384, 360], speed: [0, 0] },
  inputs: Array.from({ length: 38 }, (_, frame) =>
    input({
      move_x: frame >= 32 ? 1 : 0,
  ...
  verify(states) {
    ...
    if (
      ...
      throw new Error(
        `entity-4.7-core-super: missing BounceBlock launch grace/lift or 260/-235 Core Super: ${JSON.stringify(
          {
