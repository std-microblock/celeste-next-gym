...
export const scenario = defineScenario({
  ...
  verify(states) {
    semanticAssert(
      near(states[1]?.speed[0], 50),
      scenario.name,
      `state1=${JSON.stringify(states[1])}`,
    );
    semanticAssert(
      near(states[2]?.speed[0], 100),
      scenario.name,
      `state2=${JSON.stringify(states[2])}`,
    );
    semanticAssert(
      near(states[3]?.speed[0], 135.666_66),
      scenario.name,
      `state3=${JSON.stringify(states[3])}`,
    );
    semanticAssert(
      near(states[3]?.speed[1], -105),
      scenario.name,
      `state3=${JSON.stringify(states[3])}`,
    );
  },
