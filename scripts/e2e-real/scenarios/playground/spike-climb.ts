...
function verifySpikeClimb(states: readonly E2EState[]): void {
  semanticAssert(
    ...
    "spike-climb",
    "away-facing wall jump died on the directional spike wall",
  );
  semanticAssert(
    near(states[1]?.speed[0], -130) && near(states[1]?.speed[1], -105),
    "spike-climb",
    `launch was ${JSON.stringify(states[1]?.speed)}`,
  );
  semanticAssert(
    (states[8]?.pos[1] ?? 140) < 128,
    "spike-climb",
    "the spike-safe jump did not gain height",
  );
}
