...
export const scenario = defineScenario({
  ...
  inputs: Array.from({ length: 70 }, (_, frame) =>
    input({
  ...
  verify(states) {
    ...
    const dash = states.findIndex(
      (state, frame) => frame > released && state.state === 2,
    );
    const regrab = states.findIndex(
      (state, frame) => frame > dash && state.state === 8 && state.holding_theo,
    );
    const straightDash = states
      .slice(dash, regrab)
      .find((state) => near(state.speed[0], 240) && near(state.speed[1], 0));
    ...
    semanticAssert(
      !states[released]?.on_ground && !near(states[released]?.speed[1] ?? 0, 0),
      scenario.name,
      `release=${JSON.stringify(states[released])}`,
    );
    semanticAssert(
      dash > released && straightDash !== undefined,
      scenario.name,
