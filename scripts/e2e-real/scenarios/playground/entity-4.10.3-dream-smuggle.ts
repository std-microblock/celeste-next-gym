...
export const scenario = defineScenario({
  ...
  verify(states) {
    ...
    const inside = states.find((state) => state.state === 9 && state.holding_theo)
    const exited = states.find((state, frame) => frame > 0 && state.state !== 9 && states[frame - 1]?.state === 9 && state.holding_theo)
    if (states.some((state) => state.dead) || !pickup || !inside || !exited) {
      throw new Error(`entity-4.10.3-dream-smuggle: pickup=${!!pickup}, inside=${!!inside}, exit=${JSON.stringify(exited)}`)
    }
  },
})
