...
export const scenario = defineScenario({
  ...
  verify(states) {
    ...
    const carried = states.findIndex((state, frame) => frame > grabbed
      ...
      && state.pos[0] < 419.99)
    const released = states.findIndex((state, frame) => frame > carried
    ...
    semanticAssert(released > carried, scenario.name,
      `BounceBlock did not use the source ShakeOffPlayer release: ${JSON.stringify(states.map(pickCore))}`)
    semanticAssert(!states.some((state) => state.dead), scenario.name, 'player died during the BounceBlock side grab')
  },
})
