...
export const scenario = defineScenario({
  ...
  inputs: inputFrames(75, (frame) => ({
    move_x: (frame >= 28 && frame <= 36) || (frame >= 38 && frame < 45) ? -1 : 1,
    move_y: 0,
  ...
  verify(states) {
    ...
    const hyper = states.find((state, frame) => frame > exit && state.state === 0 && Math.abs(state.speed[0] - 325) <= 0.01)
    if (states.some((state) => state.dead) || exit < 0 || !hyper) {
      throw new Error(`entity-4.10.3.1-dream-grab-hyper: grab=${exit}, hyper=${JSON.stringify(hyper)}`)
    }
  },
})
