...
function verifyUndemoDashing(states: readonly E2EState[]): void {
  semanticAssert(
    states[1]?.state === 2 || states[1]?.state === "Dash",
    "other-5.13-undemo-omed-dashing",
    `dash did not begin on frame one: ${JSON.stringify(states.slice(0, 3).map(pickCore))}`,
  );
  semanticAssert(
    !states[1]?.ducking,
    "other-5.13-undemo-omed-dashing",
    "horizontal launch unexpectedly selected the duck collider",
  );
  const redirected = states.find(
    (state, frame) =>
      frame > 1 &&
      (state.state === 2 || state.state === "Dash") &&
      near(state.speed[0], 0) &&
      near(state.speed[1], 240),
  );
  semanticAssert(
    redirected !== undefined && !redirected.ducking,
    "other-5.13-undemo-omed-dashing",
    `freeze-time downward aim did not produce a standing downward dash: ${JSON.stringify(states.map(pickCore))}`,
  );
  const dashDir = redirected
    ? field<readonly number[]>(redirected, "DashDir")
    : undefined;
  semanticAssert(
    dashDir !== undefined && near(dashDir[0], 0) && near(dashDir[1], 1),
    "other-5.13-undemo-omed-dashing",
    `redirected DashDir is not down: ${JSON.stringify(dashDir)}`,
  );
  semanticAssert(
    !states.some((state) => state.dead),
    "other-5.13-undemo-omed-dashing",
    "player died during undemo candidate",
  );
}
