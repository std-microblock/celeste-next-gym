...
export const scenario = defineScenario({
  ...
  inputs: Array.from({ length: 35 }, (_, frame) =>
    input({
...
function verifySpikeJump(states: readonly E2EState[]): void {
  semanticAssert(
    ...
    "spike-jump",
    "the post-carry spike jump died before leaving the spike surface",
  );
  semanticAssert(
    states[27]?.on_ground === true,
    "spike-jump",
    "ZipMover did not carry the player onto a legal grounded frame after the prior spike check",
  );
  semanticAssert(
    near(states[28]?.speed[1], -105) && states[28]?.on_ground === false,
    "spike-jump",
    `jump frame was ${JSON.stringify(states[28]?.speed)}`,
  );
}
