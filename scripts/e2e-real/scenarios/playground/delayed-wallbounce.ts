...
function verifyDelayedWallbounce(states: readonly E2EState[]): void {
  ...
  semanticAssert(
    beforeJump?.state === 0 && Number(field(beforeJump, "dashAttackTimer")) > 0,
    "delayed-wallbounce",
    "jump did not occur in Normal with a lingering dash attack",
  );
  semanticAssert(
    launch?.state === 0 &&
    ...
    "delayed-wallbounce",
    `lingering up-dash produced ${JSON.stringify(launch?.speed)}`,
  );
  semanticAssert(
    Number(field(launch, "forceMoveXTimer")) === 0,
    "delayed-wallbounce",
    `SuperWallJump unexpectedly forced horizontal input: ${String(field(launch, "forceMoveXTimer"))}`,
  );
}
