...
export const scenario = defineScenario({
  ...
  recording: { primaryFor: ["3.7.8"], startFrame: 0, endFrame: 20 },
...
function verifyReverseCornerboost(states: readonly E2EState[]): void {
  ...
  semanticAssert(
    jumped?.state === 0 && jumped.facing === false,
    "reverse-cornerboost",
    "backward climb jump did not remain in Normal facing left",
  );
  semanticAssert(
    near(jumped?.stamina, 82.5) && near(jumped?.speed[0], 109.166_664),
    "reverse-cornerboost",
    `backward boost produced ${JSON.stringify(jumped?.speed)} at stamina ${jumped?.stamina}`,
  );
  semanticAssert(
    states.every((state) => !state.dead),
    "reverse-cornerboost",
    "reverse cornerboost died in its isolated booth",
  );
}
