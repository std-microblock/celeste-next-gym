...
export const scenario = defineScenario({
  target: PLAYGROUND_TARGET, status: 'candidate', tags: [], techniqueIds: ['4.26'], mapParts,
  ...
  verify(states) {
    const pickup = states.find((state, frame) => frame > 30 && state.state === 8 && state.holding_theo)
    const restored = states.find((state, frame) => frame > 30 && states[frame - 1]?.state === 8 && state.state === 0)
    semanticAssert(Boolean(pickup), scenario.name, 'updash did not regrab Theo into Pickup')
