...
export const scenario = defineScenario({
  ...
  verify(states) {
    const hit = states.findIndex(
      (state) => state.speed[0] === 0 && state.pos[0] >= 123,
    );
    ...
    const disappeared = states.findIndex(
      (state, frame) =>
        frame >= hit && cassetteBlock(state, 1)?.collidable === false,
    );
    const refunded = states.findIndex(
      (state, frame) => frame > hit && state.speed[0] > 70,
    );
    semanticAssert(
      hit > 0 &&
        disappeared >= hit &&
