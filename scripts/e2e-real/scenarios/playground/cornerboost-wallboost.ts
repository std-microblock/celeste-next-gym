...
function verifyCobwob(states: readonly E2EState[]): void {
  semanticAssert(
    Number(field(states[1], "wallSpeedRetained")) > 140 &&
      Number(field(states[1], "wallBoostTimer")) > 0.19,
    "cornerboost-wallboost",
    "neutral cornerboost did not retain speed and open the wallboost window",
  );
  semanticAssert(
    near(states[1]?.stamina, 82.5) && near(states[3]?.stamina, 110),
    "cornerboost-wallboost",
    "wallboost did not refund the climb-jump stamina",
  );
  semanticAssert(
    (states[3]?.speed[0] ?? 0) < -120 && (states[3]?.speed[0] ?? 0) > -130,
    "cornerboost-wallboost",
    `conversion speed was ${states[3]?.speed[0]}`,
  );
}
