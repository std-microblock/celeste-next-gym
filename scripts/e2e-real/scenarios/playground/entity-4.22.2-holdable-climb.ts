...
export const scenario = defineScenario({
  ...
  initial: { pos: [220, 420] },
  inputs: Array.from({ length: 50 }, (_, frame) =>
    input({
      move_y: frame === 23 ? 1 : 0,
...
function verifyHoldableClimb(states: readonly E2EState[]): void {
  ...
  const climb = states.findIndex(
    (state, frame) =>
      frame > released && state.state === 1 && !state.holding_theo,
  );
  const climbJump = states.findIndex(
    (state, frame) =>
      frame > climb && state.state === 0 && near(state.speed[1], -105),
  );
  const regrabbed = states.findIndex(
    (state, frame) =>
      frame > climbJump && state.state === 8 && state.holding_theo,
  );
  semanticAssert(
  ...
  semanticAssert(
    climb > released && climbJump > climb,
    scenario.name,
  ...
  semanticAssert(
    regrabbed > climbJump,
    scenario.name,
    `Theo was not recovered after climb jump: ${regrabbed}`,
  );
}
