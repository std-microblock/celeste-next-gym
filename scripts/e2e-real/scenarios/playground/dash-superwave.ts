...
export const scenario = defineScenario({
  ...
  tags: [],
  techniqueIds: ["2.7"],
  recording: { primaryFor: ["2.7"], startFrame: 0, endFrame: 30 },
  mapParts,
  name: "dash-superwave",
...
function verifySuperwave(states: readonly E2EState[]): void {
  ...
  semanticAssert(
    extended &&
      near(extended.speed[0], 260) &&
      near(extended.speed[1], -105) &&
      extended.dashes >= 1,
    "dash-superwave",
    "extended Super keyframe is missing",
  );
  semanticAssert(
    landing && landing.on_ground && landing.ducking && landing.speed[0] < -200,
    "dash-superwave",
    "reverse down-diagonal landing keyframe is missing",
  );
  semanticAssert(
    reverse &&
      near(reverse.speed[0], -325) &&
      near(reverse.speed[1], -52.5) &&
      reverse.dashes >= 1,
    "dash-superwave",
    "reverse Hyper keyframe is missing",
  );
}
