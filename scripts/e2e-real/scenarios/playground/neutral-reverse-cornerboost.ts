...
export const scenario = defineScenario({
  ...
  inputs: Array.from({ length: 20 }, (_, frame) =>
    input({
      move_x: frame === 0 ? 0 : 1,
...
function verifyNeutralReverseCornerboost(states: readonly E2EState[]): void {
  semanticAssert(
    near(states[1]?.speed[0], 149.166_64) && near(states[1]?.stamina, 82.5),
    "neutral-reverse-cornerboost",
    "neutral climb jump did not avoid the backward 40-speed boost",
  );
  semanticAssert(
    Number(field(states[1], "wallBoostTimer")) > 0.19,
    "neutral-reverse-cornerboost",
    `wallboost timer was ${String(field(states[1], "wallBoostTimer"))}`,
  );
  semanticAssert(
    near(states[3]?.speed[0], 125.666_66) && near(states[3]?.stamina, 110),
    "neutral-reverse-cornerboost",
    `wallboost conversion produced ${JSON.stringify(states[3]?.speed)} at stamina ${states[3]?.stamina}`,
  );
}
