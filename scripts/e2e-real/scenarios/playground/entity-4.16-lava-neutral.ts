...
export const scenario = defineScenario({
  ...
  initial: { pos: [716, 494], speed: [0, 0], state: 1, facing: 'Left', stamina: 110 },
  ...
  verify(states) {
    ...
    semanticAssert(neutral != null, scenario.name, 'neutral launch state was not recorded')
    semanticAssert((neutral.state === 0 || neutral.state === 'Normal') && near(neutral.speed[1], -105),
      scenario.name, `neutral launch=${JSON.stringify(neutral)}`)
    semanticAssert(Number(field(neutral, 'wallBoostTimer')) > 0, scenario.name,
      `wallBoostTimer=${String(field(neutral, 'wallBoostTimer'))}`)
    semanticAssert(states.slice(0, NEUTRAL_FRAME + 2).every((state) => !state.dead), scenario.name,
