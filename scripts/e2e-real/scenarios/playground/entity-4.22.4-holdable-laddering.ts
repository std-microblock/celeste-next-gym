...
export const scenario = defineScenario({
  ...
  techniqueIds: ["4.22.4"],
  mapParts,
  name: "entity-4.22.4-holdable-laddering",
  initial: { pos: [96, 400], speed: [0, -30] },
...
function verifyLaddering(states: readonly E2EState[]): void {
  const pickupStarts = states.flatMap((state, frame) =>
    state.state === 8 && states[frame - 1]?.state !== 8 ? [frame] : [],
  );
  const highest = Math.min(...states.map((state) => state.pos[1]));
  semanticAssert(
    pickupStarts.length >= 3,
    scenario.name,
    `pickup phases=${pickupStarts.join(",")}`,
  );
  semanticAssert(
    highest < (states[0]?.pos[1] ?? 0) - 16,
    scenario.name,
    `highest y=${highest}`,
  );
}
