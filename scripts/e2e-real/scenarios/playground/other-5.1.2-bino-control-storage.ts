...
export const scenario = defineScenario({
  ...
  inputs: Array.from({ length: 240 }, (_, frame) =>
    input({
  ...
  verify(states) {
    const interruptedAt = states.findIndex(
      (state) => field<boolean>(state, "boosterBoostingPlayer") === true,
    );
    semanticAssert(
      interruptedAt >= 0,
      scenario.name,
    ...
    semanticAssert(
      field<boolean>(states[interruptedAt], "lookoutInteracting") === true,
      scenario.name,
    ...
    const storedControl = states.some((state, frame) => {
      ...
      return (
        Math.abs(state.pos[0] - previous.pos[0]) > 0.01 &&
        Math.hypot(
          (camera?.[0] ?? 0) - (previousCamera?.[0] ?? 0),
          (camera?.[1] ?? 0) - (previousCamera?.[1] ?? 0),
        ) > 0.01
      );
    });
    semanticAssert(
