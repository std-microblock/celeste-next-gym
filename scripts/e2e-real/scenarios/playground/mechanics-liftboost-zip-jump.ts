...
export const scenario = defineScenario({
  ...
  tags: [],
  techniqueIds: ["1.10"],
  recording: { primaryFor: ["1.10"], startFrame: 0, endFrame: 24 },
  mapParts,
...
function verifyZipMoverLiftboost(states: readonly E2EState[]): void {
  ...
  semanticAssert(
    jumped,
    "mechanics-liftboost-zip-jump",
    "jump never inherited the upward ZipMover lift speed",
  );
  const retained = field(jumped, "lastLiftSpeed");
  semanticAssert(
    Array.isArray(retained) && retained[1] < 0,
    "mechanics-liftboost-zip-jump",
    `jump frame did not retain an upward lastLiftSpeed: ${JSON.stringify(retained)}`,
  );
  semanticAssert(
    jumped.on_ground === false && jumped.dead === false,
    "mechanics-liftboost-zip-jump",
    "liftboost jump did not leave the moving platform alive",
  );
}
