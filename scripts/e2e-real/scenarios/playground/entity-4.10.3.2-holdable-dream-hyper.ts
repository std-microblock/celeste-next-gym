...
export const scenario = defineScenario({
  ...
  verify(states) {
    const exitGrab = states.findIndex(
      (state, frame) =>
        frame > 0 &&
        state.state === 1 &&
    ...
    const released = states.findIndex(
      (state, frame) => frame > exitGrab && !state.holding_theo,
    );
    const blockedRegrab =
      released >= 0 &&
      states
    ...
    const regrab = states.find(
      (state, frame) =>
        frame > hyper && state.state === 8 && state.holding_theo,
    );
    if (
      ...
      throw new Error(
        `entity-4.10.3.2-holdable-dream-hyper: grab=${exitGrab}, release=${released}, blocked=${blockedRegrab}, hyper=${hyper}, regrab=${!!regrab}`,
      );
    }
  },
});
