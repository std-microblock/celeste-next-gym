...
function verifyNeutralDrop(states: readonly E2EState[]): void {
  ...
  const regrabbed = states.findIndex((state, frame) => frame > released && state.state === 8 && state.holding_theo)
  semanticAssert(firstPickup >= 0 && released > firstPickup, scenario.name, `pickup=${firstPickup}, release=${released}`)
  semanticAssert(near(states[released]?.speed[0], 0), scenario.name,
    `neutral drop applied horizontal recoil: ${JSON.stringify(states[released])}`)
  semanticAssert(states.slice(released, Math.min(released + 6, states.length)).every((state) => !state.holding_theo),
    scenario.name, 'CannotHold did not remain observable after the zero-force release')
  semanticAssert(regrabbed > released, scenario.name, `Theo did not remain in place for regrab: ${regrabbed}`)
}
