...
export const scenario = defineScenario({
  ...
  techniqueIds: ["4.22.3"],
  mapParts,
  name: "entity-4.22.3-jelly-neutral-jump",
  initial: { pos: [140, 420], facing: "Right" },
  inputs: Array.from({ length: 100 }, (_, frame) =>
    input({
      move_x: frame >= 30 ? 1 : 0,
      move_y: frame === 23 ? 1 : 0,
...
function verifyJellyNeutral(states: readonly E2EState[]): void {
  ...
  const neutral = states.findIndex(
    (state, frame) =>
      frame > released &&
  ...
  const regrab = states.findIndex(
    (state, frame) =>
      frame > neutral && state.state === 8 && state.holding_glider,
  );
  semanticAssert(
    released >= 0 && neutral > released,
    scenario.name,
    `release=${released}, neutral=${neutral}`,
  );
