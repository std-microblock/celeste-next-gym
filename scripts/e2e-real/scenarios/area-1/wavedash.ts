...
function verifyWavedashMinimumHeight(states: readonly E2EState[]): void {
  ...
  semanticAssert(
    landing?.state === 2 &&
      ...
      landing.dashes === 0,
    "wavedash",
    `fourteen-pixel landing did not preserve the diagonal Dash collision window: ${JSON.stringify(landing)}`,
  );
  semanticAssert(
    wavedash?.state === 0 &&
      near(wavedash.speed[0], 325) &&
      near(wavedash.speed[1], -52.5) &&
      wavedash.dashes === 1,
    "wavedash",
    `buffered landing jump did not become a refilled Hyper: ${JSON.stringify(wavedash)}`,
  );
}
