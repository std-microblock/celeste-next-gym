...
describe("map-part exporter", () => {
  it("assembles the full canonical Playground partition", () => {
    const fixture = assembleFixturePackage(
      [...COMMON_MAP_PARTS.values()],
      COMMON_MAP_PARTS,
    );
    assert.equal(fixture.rooms.length, 5);
    assert.equal(
      fixture.rooms.reduce((total, room) => total + room.solids.length, 0),
      32,
    );
    assert.equal(
      fixture.rooms.reduce((total, room) => total + room.entities.length, 0),
      85,
    );
  });

  it("computes dependency closure and emits canonical stable sorting", () => {
    const catalog = new Map<string, MapPart>([
      ["entity", entity],
      ["base", base],
    ]);
    assert.deepEqual(
      dependencyClosure([entity], catalog).map((part) => part.id),
      ["base", "entity"],
    );
    const fixture = assembleFixturePackage([entity], catalog);
    assert.equal(fixture.formatVersion, 1);
    assert.deepEqual(
      fixture.rooms[0]?.entities.map((item) => item.id),
      ["a", "b"],
    );
  ...
  it("rejects unknown dependencies, cycles, and conflicting stable ids", () => {
    assert.throws(
      () => dependencyClosure([entity], new Map([["entity", entity]])),
      /unknown/,
    );
    const cycleA = defineMapPart({
      ...base,
      id: "cycle-a",
      dependencies: ["cycle-b"],
    });
    const cycleB = defineMapPart({
      ...base,
      id: "cycle-b",
    ...
    const conflict = defineMapPart({
      ...
      rooms: [
        {
          name: "z-room",
          entities: [{ id: "a", kind: "bumper", bounds: [90, 90, 8, 8] }],
        },
      ],
    });
    assert.throws(
