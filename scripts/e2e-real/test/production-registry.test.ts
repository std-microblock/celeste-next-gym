...
describe("production scenario registry", () => {
  ...
  it("derives all target and status counts from explicit indexes", () => {
    assert.equal(registry.scenarios.length, 195);
    assert.equal(registry.byTarget.get("playground")?.length, 151);
    ...
    assert.deepEqual(registry.counts, { active: 185, candidate: 10 });
  });
  ...
  it("keeps evidence-less scenarios as opt-in candidates", () => {
    ...
    assert.equal(
      selectScenarios(registry, { target: "playground" }).some(
  ...
  it("keeps every Lookout proof in an independently named map part", () => {
    ...
    const parts = techniqueIds.map((techniqueId) => {
      const scenario = registry.scenarios.find((candidate) =>
        candidate.techniqueIds.includes(techniqueId),
      );
  ...
  it("keeps Theo regrab and slash proofs in independently named map parts", () => {
    ...
    const parts = techniqueIds.map((techniqueId) => {
      const scenario = registry.scenarios.find(
        (candidate) =>
          candidate.techniqueIds.includes(techniqueId) &&
          !candidate.tags.includes("feature:glider"),
      );
      assert.ok(scenario, `missing scenario for ${techniqueId}`);
      assert.equal(scenario.mapParts.length, 1);
    ...
    assert.deepEqual(parts, [
      "tech.entity-4.20-theo-regrab",
      "tech.entity-4.21-holdable-slash",
    ]);
  });

  it("keeps the dynamic Seeker proof in its own map part", () => {
    const seeker = registry.scenarios.find((candidate) =>
      candidate.techniqueIds.includes("4.19"),
    );
    assert.ok(seeker);
  ...
  it("keeps the TempleGate squish proof in its own map part", () => {
    const koral = registry.scenarios.find((candidate) =>
      candidate.techniqueIds.includes("4.28"),
    );
    assert.ok(koral);
  ...
  it("links the 2.8.2.1 proof and control scenarios and wires map parts by target", () => {
    assert.equal(
      registry.scenarios.filter((scenario) =>
        scenario.techniqueIds.includes("2.8.2.1"),
      ).length,
      2,
    );
    assert.equal(
      registry.scenarios
        .filter((scenario) => scenario.target.kind === "external")
        .every((scenario) => scenario.mapParts.length === 0),
      true,
    );
    assert.equal(
      registry.scenarios
        .filter((scenario) => scenario.target.kind === "playground")
        .every((scenario) => scenario.mapParts.length > 0),
      true,
    );
    assert.equal(
      registry.byTarget
        .get("area-2")
        ?.every((scenario) => scenario.room === "1"),
      true,
    );
    assert.equal(
      registry.byTarget
        .get("area-4")
        ?.every((scenario) => scenario.room === "a-02"),
      true,
    );
  });
  ...
  it("keeps every Glider proof variant in an independently named map part", () => {
    ...
    const ladderGliders =
      ladder?.mapParts[0]?.rooms[0]?.entities?.filter(
        (entity) => entity.kind === "glider",
      ) ?? [];
    assert.deepEqual(
      ladderGliders.map((entity) => entity.bounds?.[1]),
      [390, 380],
    );
  });

  it("keeps grounded ultra cancel in its own Theo-only map part", () => {
    const linked = registry.scenarios.filter((scenario) =>
      scenario.techniqueIds.includes("2.8.2.1"),
    );
    for (const scenario of linked) {
      assert.equal(scenario.mapParts.length, 1);
      assert.equal(
        scenario.mapParts[0]?.id,
        "tech.2.8.2.1.grounded-ultra-cancel",
      );
