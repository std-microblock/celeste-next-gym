...
export function playgroundScenario(name: string, overrides: Partial<ScenarioDefinition> = {}): ScenarioDefinition {
...
export function reflectedState(overrides: Partial<E2EState> = {}): E2EState {
  return {
    _frame: 0,
    _everest_fields: Object.fromEntries(Array.from({ length: 101 }, (_, index) => [`field${index}`, index])),
    pos: [19, 144],
    speed: [0, 0],
    state: 'Normal',
    facing: 'Right',
