...
describe("configuration and requests", () => {
  it("preserves environment selection and playground feature switches", () => {
    ...
    assert.deepEqual(
      ...
      ["dreamdash", "entity-4.9-dream-grab"],
    );
  ...
  it("rejects malformed flags, area ids, and target conflicts", () => {
    ...
    assert.throws(
      () => parseConfig([], { E2E_AREA_ID: "2oops" }, "D:\\repo"),
      /non-negative integer/,
    );
    assert.throws(
      () =>
        parseConfig(["--target", "area-4"], { E2E_AREA_ID: "2" }, "D:\\repo"),
      /conflicts/,
    );
    assert.throws(
      () =>
        parseConfig(
          ["--record", "--record-all", "--scenario", "dash"],
          {},
          "D:\\repo",
        ),
      /mutually exclusive/,
    ...
    assert.throws(
      () => parseConfig(["--record"], {}, "D:\\repo"),
      /exactly one/,
    );
    assert.throws(
      () => parseConfig(["--record-all", "--target", "area-1"], {}, "D:\\repo"),
      /cannot be constrained/,
    );
    assert.throws(
      () =>
        parseConfig(
          ["--timeline-regressions", "--scenario", "dash"],
          {},
          "D:\\repo",
        ),
      /cannot be combined/,
    ...
    assert.throws(
      () =>
        parseConfig(["--timeline-regressions", "--record-all"], {}, "D:\\repo"),
      /cannot be combined/,
    );
  });
  ...
  it("builds canonical simulate requests without mutating defaults", () => {
    ...
    const captured = createRequest({
      scenario,
      map: new Uint8Array([1]),
      skipTransitions: false,
      captureToken: "x".repeat(32),
    });
