...
describe('E2E isolation', () => {
  it('reserves dynamic loopback ports without launching a game', async () => {
    ...
    await assert.rejects(() => new Promise<void>((resolveConnect, reject) => {
  ...
  it('writes a per-run manifest and isolated save/temp roots', () => {
    ...
    const context = createRunContext({ repoRoot, gameInstall, modPort: 32001, httpPort: 43001, git: { branch: 'test', head: 'abc' } })
    ...
    const stored = JSON.parse(readFileSync(context.manifestPath, 'utf8')) as Record<string, unknown>
    assert.equal(stored.run_nonce, context.runNonce)
  })

  it('rejects installs outside vendor, configured Steam installs, and linked installs', { skip: process.platform !== 'win32' }, () => {
    const repoRoot = temporaryRepo()
    ...
    const linkedRepo = mkdtempSync(resolve(tmpdir(), 'celeste-e2e-link-'))
    temporaryRoots.push(linkedRepo)
    const realGame = resolve(linkedRepo, 'real-game')
    mkdirSync(realGame, { recursive: true })
    writeFileSync(resolve(realGame, 'Celeste.exe'), '')
    mkdirSync(resolve(linkedRepo, 'vendor'), { recursive: true })
    symlinkSync(realGame, resolve(linkedRepo, 'vendor', 'celeste-game'), 'junction')
    assert.throws(() => validateGameInstall({ repoRoot: linkedRepo, gameRoot: resolve(linkedRepo, 'vendor', 'celeste-game') }), /reparse\/symlink/)
  })

  it('authenticates nonce, child PID, and port', async () => {
    let pingCount = 0
    const server = net.createServer((socket) => {
      socket.once('data', () => {
        ...
        socket.end(`${JSON.stringify({
          success: pingCount >= 2,
          error: pingCount >= 2 ? undefined : 'loading',
          run_nonce: 'run-123', process_id: 456, collector_port: 32123,
        })}\n`)
      })
    })
    ...
    assert.equal((await pingEverest(address.port)).success, false)
    const response = await waitForOwnedEverest(address.port, { runNonce: 'run-123', processId: 456, port: 32123 }, 2_000)
    validateCollectorOwnership(response, { runNonce: 'run-123', processId: 456, port: 32123 })
    assert.throws(() => validateCollectorOwnership(response, { runNonce: 'wrong', processId: 456, port: 32123 }), /nonce mismatch/)
