...
describe('harness helpers', () => {
  it('creates complete immutable input frames', () => {
    const frames = inputFrames(3, (frame) => input({ move_x: frame === 1 ? 1 : 0 }))
  ...
  it('uses the pinned 0.01 core tolerance and reflected fields', () => {
    ...
    assert.deepEqual(pickCore(state), {
      frame: 7, pos: [19, 144], speed: [100, -20], state: 'Normal', facing: 'Right',
      dashes: 1, stamina: 110, on_ground: false, ducking: false, dead: true,
    })
    assert.throws(() => semanticAssert(false, 'scenario', 'guard'), /scenario.*guard/)
  })
})
