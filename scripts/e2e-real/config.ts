...
export function parseConfig(
  ...
  for (let index = 0; index < argv.length; index++) {
    ...
    if (argument === "--list") listOnly = true;
      ...
      cliNames.add(requireValue(argv, ++index, argument));
    else if (argument === "--timeline-regressions") timelineRegressions = true;
    else if (argument === "--target")
      cliTarget = parseTarget(requireValue(argv, ++index, argument));
    else if (argument === "--fixture-output")
      fixtureOutput = resolve(repoRoot, requireValue(argv, ++index, argument));
    else if (argument === "--record") recordScenario = true;
    else if (argument === "--record-tech")
      recordTechniqueIds.add(requireValue(argv, ++index, argument));
    else if (argument === "--record-all") recordAll = true;
    else throw new Error(`unknown E2E harness argument: ${argument}`);
  }

  const recordingModeCount =
    Number(recordScenario) +
    Number(recordAll) +
    Number(recordTechniqueIds.size > 0);
  if (recordingModeCount > 1)
    throw new Error(
      "--record, --record-tech, and --record-all are mutually exclusive",
    );
  const recording: RecordingCliSelection | undefined = recordScenario
    ...
    : recordAll
      ...
      : recordTechniqueIds.size > 0
        ...
        : undefined;
  if (recording && listOnly)
    throw new Error("recording modes cannot be combined with --list");
  if (recording && fixtureOutput)
    throw new Error("recording modes cannot be combined with --fixture-output");
  if (recordScenario && cliNames.size !== 1)
    throw new Error("--record requires exactly one explicit --scenario");
  if ((recordAll || recordTechniqueIds.size > 0) && cliNames.size > 0) {
    throw new Error(
      "--record-tech and --record-all cannot be combined with --scenario",
    );
  }
  if (
    timelineRegressions &&
    (cliNames.size > 0 || env.E2E_SCENARIOS !== undefined)
  ) {
    throw new Error(
      "--timeline-regressions cannot be combined with explicit scenarios",
    );
  }
  if (timelineRegressions && recording)
    throw new Error("--timeline-regressions cannot be combined with recording");
  if (recordAll && cliTarget !== undefined)
    throw new Error("--record-all cannot be constrained to one --target");
  ...
  if (envTarget === undefined)
    throw new Error(
      `unknown E2E target for area ${areaId}${areaSid ? ` (${areaSid})` : ""}`,
    );
  if (
    cliTarget &&
    (env.E2E_AREA_ID !== undefined || env.E2E_AREA_SID !== undefined) &&
    envTarget !== cliTarget
  ) {
    throw new Error(
      `--target ${cliTarget} conflicts with E2E_AREA_ID/E2E_AREA_SID (${envTarget ?? "unknown"})`,
    );
  }
  const target = cliTarget ?? envTarget;
  const effectiveAreaId = target ? TARGETS[target].areaId : areaId;
  const requestedScenarios =
    cliNames.size > 0 ? cliNames : commaSet(env.E2E_SCENARIOS);
  const disabledTags = new Set<string>();
  for (const [name, tag] of Object.entries(FEATURE_ENV))
    if (env[name] === "0") disabledTags.add(tag);
  ...
  const recordingTimeoutMs = parseBoundedMilliseconds(
    env.E2E_RECORDING_TIMEOUT_MS,
    60_000,
    "E2E_RECORDING_TIMEOUT_MS",
  );
  const recordingPollTimeoutMs = parseBoundedMilliseconds(
    env.E2E_RECORDING_POLL_TIMEOUT_MS,
    65_000,
    "E2E_RECORDING_POLL_TIMEOUT_MS",
  );
  return Object.freeze({
    ...
    mapFile:
      env.E2E_MAP_FILE ??
      DEFAULT_MAPS.get(effectiveAreaId) ??
      `${effectiveAreaId}-Unknown.bin`,
    ...
    targetExplicit:
      cliTarget !== undefined ||
      env.E2E_AREA_ID !== undefined ||
      env.E2E_AREA_SID !== undefined,
...
function parseBoundedMilliseconds(
  ...
  if (
    ...
    String(milliseconds) !== value.trim() ||
    milliseconds < 1_000 ||
