...
export function scenarioFixture(scenario: ScenarioDefinition): ScenarioFixture {
  if (scenario.target.kind !== "playground")
    throw new Error(
      `${scenario.name}: only Playground scenarios have generated fixtures`,
    );
...
export function assertNoPlaygroundMapOverride(
  scenario: ScenarioDefinition,
  mapPath?: string,
): void {
  if (scenario.target.kind === "playground" && mapPath) {
    throw new Error(
      "E2E_MAP_PATH cannot override generated per-scenario Playground maps",
    );
  }
}

export function groupScenariosByLifecycle(
  scenarios: readonly ScenarioDefinition[],
): readonly ScenarioLifecycleGroup[] {
...
export function materializePlaygroundMap(options: {
  ...
  if (
    !readFileSync(
      resolve(modRoot, "Maps", "CelesteGymPlayground", "Playground.bin"),
    ).equals(readFileSync(mapPath))
  ) {
    throw new Error(
      "staged Playground mod map differs from the scenario map used for Rust comparison",
    );
  }
  return Object.freeze({ ...generated, fixturePath, mapPath, modRoot });
}

export function stagePlaygroundMod(
  paths: HarnessPaths,
  root: string,
  mapPath: string,
): string {
  const modRoot = resolve(root, "mod");
  const stagedMap = resolve(
    modRoot,
    "Maps",
  ...
  copyFileSync(
    resolve(paths.playgroundModRoot, "everest.yaml"),
    resolve(modRoot, "everest.yaml"),
  );
