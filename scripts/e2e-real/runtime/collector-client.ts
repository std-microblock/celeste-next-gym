...
export type RecordingState = 'active' | 'ready' | 'stopped' | 'timed_out' | 'faulted' | 'finalized'
...
export interface RecordingStatus {
  ...
  readonly unpresented_update_ranges: readonly { readonly start_state_index: number; readonly end_state_index: number }[]
...
export interface RecordingControlRequest { readonly capture_token: string; readonly reason?: string }
...
export function createCollectorClient(serviceRoot: string, port: number): CollectorClient {
  ...
  return {
    ...
    recordingFinalize: async (request) => await postRecording('finalize', request),
  }


  async function postRecording(action: string, request: RecordingStartRequest | RecordingControlRequest): Promise<RecordingStatus> {
    const response = await fetch(`http://127.0.0.1:${port}/api/recording/${action}`, {
      method: 'POST', headers: { 'content-type': 'application/octet-stream' }, body: Buffer.from(codec.encode(request)),
    })
    const decoded = codec.decode(new Uint8Array(await response.arrayBuffer()))
    const body = requireRecord(decoded, 'recording response')
    if (!response.ok || body.success !== true) throw new Error(`recording ${action} failed: ${JSON.stringify(decoded)}`)
...
export function validateRecordingStatus(value: unknown): RecordingStatus {
  ...
  const states: readonly RecordingState[] = ['active', 'ready', 'stopped', 'timed_out', 'faulted', 'finalized']
  if (!states.includes(status.state as RecordingState)) throw new Error('recording status.state is invalid')
  const ranges = status.unpresented_update_ranges
  if (!Array.isArray(ranges)) throw new Error('recording status.unpresented_update_ranges is not an array')
  for (const name of ['scenario_id'] as const) requireString(status[name], `recording status.${name}`)
  for (const name of ['start_state_index', 'end_state_index', 'render_frame_count', 'repeated_presentation_count'] as const) {
    requireUnsignedInteger(status[name], `recording status.${name}`)
  }
  requireInteger(status.latest_state_index, 'recording status.latest_state_index')
  if ((status.latest_state_index as number) < -1) throw new Error('recording status.latest_state_index is less than -1')
  if ((status.end_state_index as number) < (status.start_state_index as number)) throw new Error('recording status state range is reversed')
  if (typeof status.final_state_presented !== 'boolean') throw new Error('recording status.final_state_presented is not boolean')
  for (const [index, rangeValue] of ranges.entries()) {
    const range = requireRecord(rangeValue, `recording range ${index}`)
    const start = requireUnsignedInteger(range.start_state_index, `recording range ${index}.start`)
    const end = requireUnsignedInteger(range.end_state_index, `recording range ${index}.end`)
    if (end < start) throw new Error(`recording range ${index} is reversed`)
  }
  if (status.manifest_path !== undefined) requireString(status.manifest_path, 'recording status.manifest_path')
  if (status.reason !== undefined) requireString(status.reason, 'recording status.reason')
