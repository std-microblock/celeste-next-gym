...
- 活动 Gym episode 在没有显式 `gym_step` 时完全停住真实游戏更新；网络、模型推理或并行评估耗时不会偷偷推进 Player、实体、coroutine 或房间计时器。
...
node scripts/gym-actors.mjs --actors 1 --direct-tcp --expert-replay-smoke --expert-replay-rounds 5 --soak-room 2
```
