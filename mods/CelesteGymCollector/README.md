...
- 当前每帧稳定导出 126 个字段。
- CED 的 BounceBlock 帧还导出 `reformBlockPosition`、`reformBlockCollidable`、`reformBlockColliderPresent`/`reformBlockCollider`、`reformBounceBlockState`（`Waiting=0`、`WindingUp=1`、`Bouncing=2`、`BounceEnd=3`、`Broken=4`）/`reformBounceBlockStateName`、`reformBounceBlockRespawnTimer`、`reformBounceBlockReformed`、`reformStaticMoverAlarmPresent`/`Active`/`Duration`/`TimeLeft`。顶层 `pos` 是每帧 Player Position；既有 `playerCollider`、`playerHurtbox` 与 `reformBlockCollider` 是 entity-local 几何。新 `reformBlockWorldBounds`、`reformPlayerColliderWorldBounds`、`reformPlayerHurtboxWorldBounds` 才是明确的世界坐标 `[left, top, width, height]`。`reformBlockPlayerColliderOverlaps`、`reformBlockPlayerHurtboxOverlaps` 与 `reformBlockPlayerGroundedOnSource` 也使用上述世界 AABB，作为 source body 的实时 BlockedCheck 代理。真正进入 BounceBlock source reset branch 的帧还导出 `reformResetBlockedCheckObserved`、`PreviousPosition`、`TargetPosition`、`RespawnTimer`、`ActorBlocked`、`SolidBlocked`、`BlockedCheckResult`（`actor`/`solid`/`clear`）、`BlockingActors`、`BlockingSolids`、`PostState`、`PostCollidable`、`PostPosition` 及可能的 `ProbeError`。actor/solid 项分别携带 `type`、`position`、`active`、`collidable`、`colliderPresent`、`worldBounds`，与 `BounceBlock.Update` 的先 Actor、后 Solid source check 一一对应。附属尖刺导出 `reformSpikePosition`、`reformSpikeActive`、`reformSpikeCollidable`、`reformSpikeColliderPresent`、`reformSpikeStaticMoverEnabled`；后者是真实 `StaticMover.Active`，因此可逐帧分辨 body-only reform 与 0.35 秒 Alarm 回调后的附属物启用。
