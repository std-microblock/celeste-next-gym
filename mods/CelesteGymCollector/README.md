...
- 当前每帧稳定导出 126 个字段。
- CED 的 BounceBlock 帧还导出 `reformBlockPosition`、`reformBlockCollidable`、`reformBlockColliderPresent`/`reformBlockCollider`（`[left, top, width, height]`）、`reformBounceBlockState`（`Waiting=0`、`WindingUp=1`、`Bouncing=2`、`BounceEnd=3`、`Broken=4`）/`reformBounceBlockStateName`、`reformBounceBlockRespawnTimer`、`reformBounceBlockReformed`、`reformStaticMoverAlarmPresent`/`Active`/`Duration`/`TimeLeft`。顶层 `pos` 是每帧 Player Position；`playerCollider` 与 `playerHurtbox` 是 `[left, top, width, height]` 几何。`reformBlockPlayerColliderOverlaps`、`reformBlockPlayerHurtboxOverlaps` 与 `reformBlockPlayerGroundedOnSource` 是对 source body 的实时 AABB/顶部着地 BlockedCheck 代理，不能替代实体私有 BlockedCheck 的结论。附属尖刺导出 `reformSpikePosition`、`reformSpikeActive`、`reformSpikeCollidable`、`reformSpikeColliderPresent`、`reformSpikeStaticMoverEnabled`；后者是真实 `StaticMover.Active`，因此可逐帧分辨 body-only reform 与 0.35 秒 Alarm 回调后的附属物启用。
