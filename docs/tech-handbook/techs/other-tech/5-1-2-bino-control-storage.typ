...
#tech(
  ...
  e2e-evidence: none,
  candidate-e2e: evidence(path: [scripts/e2e-real/scenarios/playground/other-5.1.2-bino-control-storage.ts; scripts/e2e-real/scenarios/lookout-parts.ts; mods/CelesteGymCollector/Source/SnapshotCapture.cs; mods/CelesteGymCollector/Source/CelesteGymCollectorModule.cs; crates/celeste-physics/src/sim.rs], symbol: [other-5.1.2-bino-control-storage; TECH_OTHER_5_1_2_BINO_CONTROL_STORAGE; boosterBoostingPlayer; prepare_lookout_player; InstallScriptedButtons], note: [2026-07-28 run `2026-07-28T10-31-27.413Z-91484-db181161-a7f9-4748-9337-4bbfc2482dfd` 已认证并清理，Booster/interacting 与 Normal+camera 语义通过到最后一步，但 frame 180 jump 未清除 interaction。后续候选修复 `a7fea5b`（DummyWalk 仅首帧写 StDummy）在受控 run `2026-07-28T16-33-53.641Z-91748-1c5bfa95-e544-4364-a3c2-2799f5232b14` 以物理 `vendor/celeste-game`、隔离 save/tmp、动态端口、nonce+child PID 握手完成并清理；首差改为 frame 18：Rust `(510,496), Boost, vx=16.667`，Everest `(511,496), Boost, vx=33.333`，其余九字段相同。281 帧仍失败（max position 62，max speed 240），保持 candidate。最新候选 `ecd751b` 的受控 run `2026-07-28T17-05-08.513Z-103452-ea3fed4f-d29a-4927-9d14-f0aad60872dc` 同样使用物理 vendor 游戏、隔离 save/tmp、动态端口和 nonce+精确 child PID 握手，且受控清理完成；在逐帧比较前，语义门再次确认 jump exit 未清除 `Lookout.interacting`，故不可转正。修复后 `7db6c87` 的 run `2026-07-28T17-12-38.435Z-86848-e82e8a24-de6e-4ad0-a788-21c90af0a726` 以同一受控协议完成：MenuCancel 语义门已通过，但首差为 frame 20，Rust `Dummy (512,496)`、Everest `Boost (513,496)`；241 帧最大位置误差 62、最大速度误差 240，保持 candidate。]),
)
