...
#tech(
  ...
  e2e-evidence: none,
  candidate-e2e: evidence(path: [scripts/e2e-real/scenarios/playground/other-5.1.3-bino-interaction-storage.ts; scripts/e2e-real/scenarios/lookout-parts.ts; mods/CelesteGymCollector/Source/CelesteGymCollectorModule.cs; crates/celeste-physics/src/sim.rs], symbol: [other-5.1.3-bino-interaction-storage; TECH_OTHER_5_1_3_BINO_INTERACTION_STORAGE; LookoutRemoved; bino_interaction_storage_uses_a_native_booster_after_dummy_walk], note: [DummyWalk 不在 Player.InControl，不能直接过图。候选 fixture 先走到 x=940，再由房内原生 Booster 打断已完成的 DummyWalk，随后在 Lookout 相机协程仍运行时用 Normal 移动跨越边界；collector 在 `Lookout.Removed` 的 orig 前捕获 `interacting`，并在 orig 后记录 StNormal。保持 candidate，待真实 Everest 九字段重跑。]),
)
