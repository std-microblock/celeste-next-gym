...
#tech(
  ...
  e2e-evidence: none,
  candidate-e2e: evidence(path: [scripts/e2e-real/scenarios/playground/other-5.1.3-bino-interaction-storage.ts; scripts/e2e-real/scenarios/lookout-parts.ts; mods/CelesteGymCollector/Source/CelesteGymCollectorModule.cs; crates/celeste-physics/src/sim.rs], symbol: [other-5.1.3-bino-interaction-storage; TECH_OTHER_5_1_3_BINO_INTERACTION_STORAGE; LookoutRemoved; bino_interaction_storage_uses_a_native_booster_after_dummy_walk], note: [2026-07-29 两次独立 Everest 运行均失败语义门：首次确认 Lookout Position 为 x=940、DummyWalk 停在 Player X=932，center X=952 的 Booster 未触发；按 X-8 校准并重跑后仍失败，trace 显示继承的 playground.base 右墙 x=936 将 Player 固定在 x=932。候选改为专用无右墙双房 fixture，并保持原生 Booster PlayerCollider；待独立重跑。]),
)
