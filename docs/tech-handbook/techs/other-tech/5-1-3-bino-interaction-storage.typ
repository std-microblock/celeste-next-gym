...
#tech(
  ...
  e2e-evidence: none,
  candidate-e2e: evidence(path: [scripts/e2e-real/scenarios/playground/other-5.1.3-bino-interaction-storage.ts; scripts/e2e-real/scenarios/lookout-parts.ts; mods/CelesteGymCollector/Source/CelesteGymCollectorModule.cs; crates/celeste-physics/src/sim.rs; Celeste/Player/Player.cs; Celeste/Booster.cs], symbol: [other-5.1.3-bino-interaction-storage; TECH_OTHER_5_1_3_BINO_INTERACTION_STORAGE; LookoutRemoved; bino_interaction_storage_uses_a_native_booster_after_dummy_walk; Player.Update; Booster.Booster], note: [2026-07-29 前两次独立 Everest 运行均未进入原生 Booster：首次确认 Lookout Position 为 x=940、DummyWalk 停在 Player X=932，center X=952 的 Booster 未触发；第二次 trace 显示继承的 playground.base 右墙 x=936 将 Player 固定在 x=932。专用无右墙双房 fixture 的第三次重跑在 frame 8 逐帧门失败：Rust 已在 x=920 进入 Boost，Everest 仍为 Dummy（speed 64）。源码确认 `Booster` 使用 `Circle(10, 0, 2)` 的 `PlayerCollider`，而 `Player.Update` 在遍历所有 PlayerCollider 前临时换入 8x9 hurtbox；Rust 先前误用 8x11 movement collider，使边缘圆形相交早一帧。第四次运行（bef2691）已将首次 Booster overlap 对齐到 frame 9，但完整比较仍在 frame 10 失败：Rust Boost 为 x=921、Everest 为 x=922，随后误差扩大至 position 77、speed 240；保持 candidate。]),
)
