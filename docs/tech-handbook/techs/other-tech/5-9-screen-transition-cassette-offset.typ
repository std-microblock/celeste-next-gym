...
#tech(
  ...
  e2e-evidence: evidence(path: [scripts/e2e-real/scenarios/playground/other-5.9-screen-transition-cassette-offset.ts; scripts/e2e-real/scenarios/cassette-spinner-parts.ts; mods/CelesteGymCollector/Source/SnapshotCapture.cs], symbol: [other-5.9-screen-transition-cassette-offset; TECH_OTHER_5_9_TRANSITION_CASSETTE_OFFSET; SnapshotCapture.Capture], note: [2026-07-28 隔离真实 Everest run `2026-07-28T13-44-36.305Z-97860-dcd51a8c-26d9-4d91-943f-d99bb3dcb706` 完成 nonce/PID 认证和受控清理；91 帧九字段逐帧匹配（最大 position 误差 0.000019、speed 误差 0）。transition 中 collector 同帧看到四个 block：旧／新 index-0 分别为 Y=18/-46 且非碰撞，index-1 分别为 Y=16/-48 且可碰撞；这证明两房载入期间保持各自 cassette 相位和 offset。]),
  candidate-e2e: none,
)
