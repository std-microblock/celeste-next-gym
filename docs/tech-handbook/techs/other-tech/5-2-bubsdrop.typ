...
#tech(
  ...
  e2e-evidence: none,
  candidate-e2e: evidence(path: [scripts/e2e-real/scenarios/playground/other-5.2-bubsdrop.ts; scripts/e2e-real/scenarios/bubs-parts.ts; mods/CelesteGymCollector/Source/SnapshotCapture.cs], symbol: [other-5.2-bubsdrop; TECH_OTHER_5_2_BUBSDROP; additionalSpawns; sessionRespawnPoint], note: [独立双房 MapPart 在上房放置相邻 wall 与 JumpThru，并在旧房注入第二个 `(440,496)` spawn；脚本验证 -105 auto-jump、`(-130,-105)` 墙跳、y>0 回旧房以及 collector 的 `[440,496]` Session.RespawnPoint。等待主工作区真实 Everest trace 后转正。]),
)
