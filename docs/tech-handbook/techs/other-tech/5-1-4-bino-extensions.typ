...
#tech(
  ...
  e2e-evidence: none,
  candidate-e2e: evidence(path: [scripts/e2e-real/scenarios/playground/other-5.1.4-bino-extensions.ts; scripts/e2e-real/scenarios/lookout-parts.ts; mods/CelesteGymCollector/Source/CelesteGymCollectorModule.cs], symbol: [other-5.1.4-bino-extensions; TECH_OTHER_5_1_4_BINO_EXTENSIONS; InstallScriptedButtons], note: [旧 trace `.tmp/e2e-other-5.1.4-bino-extensions-trace.json` 在 f500 已抵达末节点 `node=2, percent=1`，至 f720 仍为 `Dummy + interacting`，证明终点不是自动退出。候选改为 f520 以 jump 映射 `MenuCancel`，再保留 summit 的一秒 wipe 等待。候选 SHA `a5f765701cbc942fd1abe810db7e35f1e9a9150f` 的受控 Everest run `2026-07-28T19-41-39.447Z-114340-95f51b7f-1cd4-499c-a6a1-c1840a1a62db` 将 trace 写入 `.tmp/e2e-other-5.1.4-bino-extensions-trace.json`，comparator 在 f2 即失败（游戏 `Dummy`、Rust `Normal`；最大 position=7、speed=105.000420）；manifest 记录 runner 已完成受控 cleanup。第二次受控 run 的 manifest 为 `.tmp/e2e-runs/2026-07-28T19-49-13.025Z-71404-1c940c91-9af6-4287-b538-3214dd8cffac/manifest.json`，候选 SHA `3c853cd727ec5ec177958ec432a08b18e907cb63` 使用已验证的物理 `vendor/celeste-game`、nonce 与子 PID `38836` 握手、动态端口 `60313/60314`，并完成 cleanup；comparator 首个不一致在 f581（Rust `Dummy`、游戏 `Normal`，position/speed 均为 0）。游戏已在 f520 的真实 `MenuCancel` 后退出，模拟器仍停在 `Dummy`。故真实 E2E 尚未通过，保持 candidate。]),
)
