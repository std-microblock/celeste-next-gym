...
#tech(
  ...
  e2e-evidence: none,
  candidate-e2e: evidence(path: [scripts/e2e-real/scenarios/playground/other-5.1.4-bino-extensions.ts; mods/CelesteGymCollector/Source/CelesteGymCollectorModule.cs; crates/celeste-physics/src/sim.rs], symbol: [other-5.1.4-bino-extensions; InstallScriptedButtons; advance_node_lookout_camera; bino_extensions_follow_nodes_and_run_long_distance_exit_wipe], note: [2026-07-29 的 Everest trace 在 frame 453 到达 node 2、nodePercent=1 后仍保持 Dummy/interacting；summit endpoint 只钳制节点，不自动退出。对 `f65fc1b` 的重跑在 frame 460 注入 portable jump 后至 frame 720 仍为 Dummy/interacting，未开始 long-distance wipe，语义门失败。`LookRoutine` 的 source 条件是 `Input.MenuCancel.Pressed || Input.Dash.Pressed`；runner 在 Input 更新后才安装 CurrentInput，raw MenuCancel edge 已错过，候选改为使用 collector-owned jump buffer，待独立 Everest 重跑。]),
)
