...
#tech(
  ...
  e2e-evidence: none,
  candidate-e2e: evidence(path: [scripts/e2e-real/scenarios/playground/entity-4.18.2.1-cassoosted-fuper.ts; scripts/e2e-real/scenarios/cassette-spinner-parts.ts; mods/CelesteGymCollector/Source/SnapshotCapture.cs], symbol: [entity-4.18.2.1-cassoosted-fuper; TECH_ENTITY_4_18_2_1_CASSOOSTED_FUPER; SnapshotCapture.Capture], note: [独立 Feather + 双颜色 CassetteBlock MapPart 已保留；collector 现在逐帧输出 cassette runtime，候选除 `(273.33334,-105)` Feather Super 外还要求 index-0 reform 在相邻帧发生。此前 trace 的 `(224.16676,-105)` 仍不满足该门槛；尚未使用新观测字段重跑，故不转正。]),
)
