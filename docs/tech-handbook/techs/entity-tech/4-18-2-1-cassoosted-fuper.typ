...
#tech(
  ...
  test-evidence: evidence(path: [crates/celeste-physics/src/sim.rs], symbol: [cassoosted_fuper_combines_grounded_starfly_jump_and_same_frame_reform; cassoosted_fuper_fixture_aligns_first_starfly_jump_with_tempo_three_reform], note: [单帧回归同时断言 StarFly→Normal 的 `(273.33334,-105)` 与 Cassette 恢复碰撞、上移到 source position、玩家额外纵向位移；候选 fixture 回归从 fresh Feather/manager 运行，锁定 state 29 的 grounded Feather Super 与 index-0 reform 同帧。]),
  e2e-evidence: none,
  candidate-e2e: evidence(path: [scripts/e2e-real/scenarios/playground/entity-4.18.2.1-cassoosted-fuper.ts; scripts/e2e-real/scenarios/cassette-spinner-parts.ts; mods/CelesteGymCollector/Source/SnapshotCapture.cs], symbol: [entity-4.18.2.1-cassoosted-fuper; TECH_ENTITY_4_18_2_1_CASSOOSTED_FUPER; SnapshotCapture.Capture], note: [2026-07-28 隔离真实 Everest run `2026-07-28T14-10-09.822Z-74888-88bf63e0-49b5-4033-b930-27c41693f908` 完成物理 vendor 安装校验、nonce/精确 child PID 认证及受控清理。frame 29 的 index-0 已恢复到 `(304,493)` 且 `collidable=true`，同帧 Feather 退出为 Normal，但快照为 `(359,490)/(273.3333,-164.99988)`、`on_ground=false`，而非要求的 grounded `(273.33334,-105)`；semantic gate `fuper=-1`。未取得九字段和 fuper/reform telemetry 通过，故不转正。]),
)
