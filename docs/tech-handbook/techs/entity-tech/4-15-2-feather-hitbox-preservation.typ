...
#tech(
  ...
  source-evidence: evidence(
    ...
    note: [Bounce 先缓存当前 Collider，再把状态切回 Normal；StarFlyEnd 在状态回调中恢复普通 collider/hurtbox，随后 Bounce 又写回缓存的 starFlyHitbox，因此形成 StarFly collider + normal hurtbox。],
    snippet: raw(block: true, lang: "cs", "var was = Collider;\nCollider = normalHitbox;\nMoveVExact((int)(fromY - Bottom));\nStateMachine.State = StNormal; // runs StarFlyEnd\n...\nCollider = was;"),
  ),
