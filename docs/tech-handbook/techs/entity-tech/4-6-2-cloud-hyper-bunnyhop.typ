...
#tech(
  ...
  e2e-evidence: none,
  candidate-e2e: evidence(path: [scripts/e2e-real/scenarios/playground/entity-4.6.2-cloud-hyper-bunnyhop.ts; mods/CelesteGymCollector/Source/SnapshotCapture.cs; crates/celeste-physics/examples/compare_real_trace.rs], symbol: [entity-4.6.2-cloud-hyper-bunnyhop; SnapshotCapture.Capture; compare_real_trace], note: [独立 MapPart 使用真实 Cloud 与对齐到其 18px 顶点位移的 8px 网格平台；平台左缘后移一个网格，避免反向 Hyper 离云时额外撞到平台侧面。2026-07-28 的旧 46 帧比较在第 35 帧有 1px Y 首差（Rust=414、Everest=415）；复核发现旧 trace 只序列化 `Engine.DeltaTime` 的十进制 `0.0166667`，重读为 f32 后已不是原帧位值，f30 起产生 0.000290 的 Y speed 偏移并使 f35 movementCounter 错跨 -0.5。Collector 现另存 `SingleToInt32Bits`，比较器优先按位重放；旧 trace 仍只能作诊断，待精确位值的真实 Everest 九字段重跑，故保持 candidate。]),
)
