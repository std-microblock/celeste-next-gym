...
#tech(
  ...
  e2e-evidence: none,
  candidate-e2e: evidence(path: [scripts/e2e-real/scenarios/playground/entity-4.6.2-cloud-hyper-bunnyhop.ts; mods/CelesteGymCollector/Source/SnapshotCapture.cs; crates/celeste-physics/examples/compare_real_trace.rs], symbol: [entity-4.6.2-cloud-hyper-bunnyhop; SnapshotCapture.Capture; compare_real_trace], note: [独立 MapPart 使用真实 Cloud 与对齐到其 18px 顶点位移的 8px 网格平台；平台左缘后移一个网格，避免反向 Hyper 离云时额外撞到平台侧面。2026-07-28 的精确位值重跑仍在第 35 帧有 1px Y 首差（Rust=414、Everest=415），position 最大误差 1、speed 最大误差 0.000580。每帧 `engineDeltaTimeBits` 均为 `0x3c88889a`（f32 `0.0166666992`），恰与旧 JSON `0.0166667` 重读值相同；比较器已按位重放，故这次差异不能归因于十进制序列化。Y speed 从 f30 起保持 0.000290 偏移并使 f35 movementCounter 错跨 -0.5；仍须对齐 Cloud/Player 的更早帧状态，保持 candidate。]),
)
