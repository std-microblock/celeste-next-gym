...
#tech(
  ...
  e2e-evidence: none,
  candidate-e2e: evidence(path: [scripts/e2e-real/scenarios/playground/entity-4.6.2-cloud-hyper-bunnyhop.ts; mods/CelesteGymCollector/Source/SnapshotCapture.cs; crates/celeste-physics/examples/compare_real_trace.rs], symbol: [entity-4.6.2-cloud-hyper-bunnyhop; SnapshotCapture.Capture; compare_real_trace], note: [独立 MapPart 使用真实 Cloud 与对齐到其 18px 顶点位移的 8px 网格平台；平台左缘后移一个网格，避免反向 Hyper 离云时额外撞到平台侧面。2026-07-28 重跑的 46 帧真实比较仍在第 35 帧有 1px Y 首差（Rust=414、Everest=415），speed 最大误差 0.000580；两者均记录/重放了 `Engine.DeltaTime=0.0166667`，故不是未捕获的帧时间差，而是 Cloud/Player movementCounter 的浮点顺序仍未对齐。保持 candidate。]),
)
