...
#tech(
  ...
  e2e-evidence: none,
  candidate-e2e: evidence(path: [scripts/e2e-real/scenarios/playground/entity-4.6.2-cloud-hyper-bunnyhop.ts; mods/CelesteGymCollector/Source/SnapshotCapture.cs; crates/celeste-physics/examples/compare_real_trace.rs], symbol: [entity-4.6.2-cloud-hyper-bunnyhop; SnapshotCapture.Capture; compare_real_trace], note: [独立 MapPart 使用真实 Cloud 与对齐到其 18px 顶点位移的 8px 网格平台；平台左缘后移一个网格，避免反向 Hyper 离云时额外撞到平台侧面。旧 46 帧比较在第 35 帧有 1px Y 首差（Rust=414、Everest=415），而 speed 最大误差仅 0.000580，恰好跨过 movementCounter 的 -0.5 取整边界。Collector 现记录每帧 Engine.DeltaTime，比较器按该原始位模式重放，待真实 Everest 九字段重跑，故保持 candidate。]),
)
