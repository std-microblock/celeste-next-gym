...
#tech(
  ...
  test-evidence: evidence(path: [crates/celeste-physics/src/sim.rs], symbol: [dash_pickup_runs_before_the_dash_coroutine_samples_direction / theovator_regrabs_after_updash_speed_is_live_and_restores_it_after_pickup / jellyvator_regrabs_updash_and_restores_vertical_speed], note: [DashUpdate 的 Holdable 检查先于 DashCoroutine 初始 yield 后的 DashDir 赋值；随后两项回归验证 Theo/Jelly 重抓分别恢复缓存的上冲速度。]),
  e2e-evidence: none,
