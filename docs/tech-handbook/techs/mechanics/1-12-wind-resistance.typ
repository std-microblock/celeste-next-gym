...
#tech(
  ...
  source-evidence: evidence(
    ...
    symbol: [Player.WindMove],
    snippet: raw(block: true, lang: "cs", "if (!CollideCheck<Solid>(Position + Vector2.UnitX * -Math.Sign(move.X) * WindWallDistance)) {\n    if (Ducking && onGround)\n        move.X *= DuckWindMult;\n    MoveH(move.X);\n}"),
    note: [水平风在接地蹲伏时乘 DuckWindMult=0，并保留墙体遮挡检查。],
  ),
