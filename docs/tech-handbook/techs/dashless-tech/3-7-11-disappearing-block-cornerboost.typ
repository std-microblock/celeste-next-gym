...
#tech(
  ...
  test-evidence: evidence(path: [crates/celeste-physics/src/sim.rs], symbol: [disappearing_cassette_cornerboost_restores_retained_speed_after_entity_phase; disappearing_cassette_cornerboost_fixture_times_hit_clear_and_refund], note: [基础回归让 index-0 cassette 在第一帧挡住 120 速度并记录 retained speed；同帧 beat 8 只写 `Activated=false`，第二帧实体 phase 才清除碰撞，第三个 Player.Update 在仍未到期的 0.06 秒窗口返还速度。候选 fixture 回归再锁定 fresh manager 的 tempo=3：state 29 撞墙并失活，state 30 恢复为 `(126,496)/(90,0)`。]),
  e2e-evidence: none,
  candidate-e2e: evidence(path: [scripts/e2e-real/scenarios/playground/dashless-3.7.11-disappearing-block-cornerboost.ts; scripts/e2e-real/scenarios/cassette-spinner-parts.ts; mods/CelesteGymCollector/Source/SnapshotCapture.cs], symbol: [dashless-3.7.11-disappearing-block-cornerboost; TECH_DASHLESS_3_7_11_DISAPPEARING_BLOCK_CORNERBOOST; SnapshotCapture.Capture], note: [2026-07-28 隔离真实 Everest run `2026-07-28T14-09-05.707Z-49972-948a9158-25cb-4234-887b-5f8f9e48d839` 完成物理 vendor 安装校验、nonce/精确 child PID 认证及受控清理。tempo=3 场景仍未发生墙碰撞：frame 27 为 `(123,496)/(66.666794,0)`，frame 28 块 index-1 已 `collidable=false`，frame 29 玩家直接到 `(126,496)/(90,0)`；semantic gate `hit=-1, disappeared=28, refunded=29`。未取得九字段差分通过，故不转正。]),
)
