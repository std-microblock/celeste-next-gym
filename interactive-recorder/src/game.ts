...
import { createHarnessPaths, prepareMods } from '../../scripts/e2e-real/runtime/prepare-mods.js'
...
export async function recordGame(options: { readonly maxFrames?: number } = {}): Promise<string> {
  const recorderRoot = resolve(dirname(fileURLToPath(import.meta.url)), '..')
  const repoRoot = resolve(recorderRoot, '..')
  const mainWorkspaceLine = captureCommand('git', ['worktree', 'list', '--porcelain'], repoRoot)
    .split(/\r?\n/)
    .find((line) => line.startsWith('worktree '))
  const mainWorkspace = mainWorkspaceLine?.slice('worktree '.length)
  if (!mainWorkspace || resolve(mainWorkspace) !== repoRoot) {
    throw new Error(`interactive game recording must run from the main workspace: ${mainWorkspace ?? '(unknown)'}`)
  }
  const paths = createHarnessPaths(repoRoot)
  const gameInstall = validateGameInstall({ repoRoot, gameRoot: paths.gameRoot })
  const git = {
    branch: captureCommand('git', ['branch', '--show-current'], repoRoot),
    head: captureCommand('git', ['rev-parse', 'HEAD'], repoRoot),
  }
  if (!git.branch) throw new Error('interactive game recording requires a named branch, not detached HEAD')
  const dirty = captureCommand('git', ['status', '--porcelain'], repoRoot)
  if (dirty) throw new Error(`interactive game recording requires a clean main workspace:\n${dirty}`)
  ...
  const recordingRoot = resolve(recorderRoot, 'recordings', `${recordingId}-${randomUUID()}`)
  ...
  try {
    ...
    updateRunManifest(context, { status: 'starting-interactive-game', recording_root: recordingRoot })
    ...
    updateRunManifest(context, { status: 'interactive-recording', interactive_recording: status })
    console.log('Playground 已进入并开始逐 Player.Update 录制。回到此终端按 Enter 停止并保存。')
    const prompt = createInterface({ input: process.stdin, output: process.stdout })
    try {
      await Promise.race([
        prompt.question(''),
        new Promise<never>((_, reject) => game!.once('exit', () => reject(new Error('Celeste exited before the recording was stopped')))),
      ])
    } finally {
      prompt.close()
    }
    ...
    if (status.state !== 'stopped' || !status.trace_path) throw new Error('game did not finalize an interactive trace')
    const rawTrace = containedPath(recordingRoot, status.trace_path)
    const mapOutput = resolve(recordingRoot, 'maps', 'CelesteGymPlayground', 'Playground.bin')
    const fixtureOutput = resolve(recordingRoot, 'maps', 'playground.map.fixture.json')
    mkdirSync(dirname(mapOutput), { recursive: true })
    copyFileSync(resolve(recorderRoot, 'maps', 'CelesteGymPlayground', 'Playground.bin'), mapOutput)
    copyFileSync(resolve(recorderRoot, 'maps', 'playground.map.fixture.json'), fixtureOutput)
    const mapHash = createHash('sha256').update(readFileSync(mapOutput)).digest('hex')
    const trace = JSON.parse(readFileSync(rawTrace, 'utf8')) as { map: Record<string, unknown> }
    ...
    writeFileSync(resolve(recordingRoot, 'manifest.json'), `${JSON.stringify({
      version: 1,
      trace: 'trace.json',
    ...
    updateRunManifest(context, { status: 'completed', trace: traceOutput, frames: status.frame_count })
    console.log(`已保存 ${status.frame_count} 个输入帧和 ${status.state_count} 个状态：${traceOutput}`)
    return traceOutput
  } catch (error) {
    runError = error
...
async function waitUntilActive(
  ...
  do {
    if (game.exitCode !== null) throw new Error('Celeste exited while loading Playground')
    const status = await sendInteractive(port, { command: 'interactive_status', ...authenticated })
    if (status.state === 'active') return status
    await new Promise<void>((resolveWait) => setTimeout(resolveWait, 100))
  } while (Date.now() < deadline)
  throw new Error('Playground player did not become recordable within 30 seconds')
}

async function sendInteractive(port: number, request: unknown): Promise<InteractiveStatus> {
  const response = await new Promise<CollectorResponse>((resolveResponse, reject) => {
    ...
    const timer = setTimeout(() => finish(() => reject(new Error('interactive collector request timed out'))), 60_000)
    timer.unref()
    ...
    socket.on('data', (chunk) => {
      ...
      if (newline < 0) return
      finish(() => {
        try { resolveResponse(JSON.parse(data.slice(0, newline)) as CollectorResponse) }
        catch (error) { reject(new Error(`invalid interactive collector response: ${String(error)}`)) }
      })
    })
    socket.once('error', (error) => finish(() => reject(error)))
    socket.once('end', () => finish(() => reject(new Error('interactive collector closed without a response'))))
  })
  if (response.success !== true || !response.interactive_recording) {
    throw new Error(response.error ?? 'interactive collector returned no status')
  }
...
function containedPath(root: string, relativePath: string): string {
  ...
  if (rel.startsWith('..') || resolve(root, rel) !== candidate) throw new Error('trace path escapes recording root')
  return candidate
}

function cleanupOwned(game: ChildProcess | undefined, identity: ProcessIdentity | undefined): boolean {
  if (!identity) {
    if (game?.pid && game.exitCode === null) console.warn(`refusing to terminate PID ${game.pid}: no recorded process identity`)
    return false
  }
