...
if (command === 'compare') {
  ...
  if (!actualPath || !expectedPath) {
    throw new Error('usage: npm run compare -- <actual.trace.json> <expected.trace.json> [tolerance]')
  }
  const actual = loadTrace(actualPath)
  const expected = loadTrace(expectedPath)
  const result = compareTraces(actual, expected, toleranceRaw === undefined ? undefined : Number(toleranceRaw))
