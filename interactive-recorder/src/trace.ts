...
export function validateTrace(value: unknown): FrameTrace {
  ...
  if (trace.source !== "web" && trace.source !== "game")
    throw new Error("trace.source must be web or game");
  record(trace.map, "trace.map");
  if (!Array.isArray(trace.inputs) || !Array.isArray(trace.states))
    throw new Error("trace inputs/states must be arrays");
  if (trace.states.length !== trace.inputs.length + 1)
    throw new Error("trace states length must equal inputs length + 1");
...
export function compareTraces(
  ...
  for (let frame = 0; frame < compared; frame += 1) {
    ...
    if (normalizeState(left.state) !== normalizeState(right.state))
      fields.push("state");
    if (normalizeFacing(left.facing) !== normalizeFacing(right.facing))
      fields.push("facing");
    if (left.dashes !== right.dashes) fields.push("dashes");
    if (Math.abs(left.stamina - right.stamina) > tolerance)
      fields.push("stamina");
  ...
  if (
    firstMismatch === null &&
    actual.states.length !== expected.states.length
  ) {
...
function validateInput(value: unknown, frame: number): void {
  ...
  if (
    ![-1, 0, 1].includes(input.move_x as number) ||
    ![-1, 0, 1].includes(input.move_y as number)
  ) {
  ...
  for (const key of [
    ...
    if (typeof input[key] !== "boolean")
      throw new Error(`inputs[${frame}].${key} must be boolean`);
  }
}
...
function validateState(value: unknown, frame: number): void {
  ...
  if (typeof state.state !== "string" && !Number.isInteger(state.state))
    throw new Error(`states[${frame}].state is invalid`);
  if (![true, false, "Left", "Right", -1, 1].includes(state.facing as never))
    throw new Error(`states[${frame}].facing is invalid`);
  for (const key of ["dashes", "stamina"]) {
    if (typeof state[key] !== "number" || !Number.isFinite(state[key]))
      throw new Error(`states[${frame}].${key} is invalid`);
  }
  for (const key of ["on_ground", "ducking", "dead"]) {
    if (typeof state[key] !== "boolean")
      throw new Error(`states[${frame}].${key} must be boolean`);
  }
}
